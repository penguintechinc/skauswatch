        ...
        with self._lock:
            ...
            for task_id, task in self._tasks.items():
                if (task.get('status') in ['completed', 'failed'] and 
                    task.get('end_time', 0) < current_time - max_age_seconds):
...
class SmartThreadPoolExecutor(ThreadPoolExecutor):
    ...
    def submit_task(self, 
        ...
        self.task_tracker.add_task(task_id, {
    ...
    def _wrap_task_execution(self, 
                           func: Callable[..., T], 
                           task_id: str, 
                           timeout: float) -> Callable[..., T]:
            ...
            try:
                ...
                with self.metrics_lock:
                    ...
                    if len(self.task_metrics.execution_times) > 1000:
                        self.task_metrics.execution_times = self.task_metrics.execution_times[-1000:]
                    ...
                    if self.task_metrics.execution_times:
                        self.task_metrics.average_execution_time = (
                            sum(self.task_metrics.execution_times) / 
    ...
    def _check_auto_scaling(self) -> None:
        ...
        with self.metrics_lock:
            queue_utilization = min(1.0, self.task_metrics.active_tasks / max(1, self.config.queue_size))
            
        current_workers = self._threads.__len__() if hasattr(self, '_threads') else self._target_workers
    ...
    def get_metrics(self) -> PoolMetrics:
        ...
        with self.metrics_lock:
            ...
            return PoolMetrics(
                active_threads=getattr(self, '_threads', {}) and len(self._threads) or 0,
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            hung_tasks = [
                ...
                if current_time - task.get('start_time', current_time) > self.config.task_timeout * 2
            ]
            
            if hung_tasks:
                logger.warning(f"Found {len(hung_tasks)} potentially hung tasks in {self.pool_name}")
...
class ThreadPoolManager:
    ...
    def create_pool(self, 
                   name: str, 
                   task_type: TaskType = TaskType.MIXED,
                   config: Optional[ThreadPoolConfig] = None) -> SmartThreadPoolExecutor:
...
class AsyncThreadPoolExecutor:
    ...
    async def run_in_thread(self,
        ...
        _, result = await self.pool_manager.submit_task(
            pool_name, func, *args, 
            task_type=task_type, timeout=timeout, **kwargs
        )
        return result
...
def thread_pool_task(pool_name: str = "default",
                    task_type: TaskType = TaskType.MIXED,
                    timeout: Optional[float] = None,
                    pool_manager: Optional[ThreadPoolManager] = None) -> Callable[[F], F]:
