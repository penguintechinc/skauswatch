...
from typing import (
    ...
    AsyncContextManager, Set, NamedTuple
)
    ...
    async def check_rate_limit(self, key: str) -> RateLimitResult:
        ...
        async with self.lock:
            ...
            if not allowed and window:
                retry_after = window[0].timestamp + self.config.window_seconds - current_time
...
class FixedWindowRateLimiter(BaseRateLimiter):
    ...
    async def cleanup(self) -> int:
        ...
        async with self.lock:
            current_time = time.time()
            current_window_start = int(current_time // self.config.window_seconds) * self.config.window_seconds
            
            keys_to_remove = []
            for key, window in self.windows.items():
                # Remove windows that are more than one window period old
                if window['window_start'] < current_window_start - self.config.window_seconds:
                    keys_to_remove.append(key)
...
class DistributedRateLimiter(BaseRateLimiter):
    ...
    async def _ensure_connection(self) -> None:
        ...
        if self.redis_client is None:
            if self.config.redis_url:
                ...
                raise ValueError("redis_url must be provided for DistributedRateLimiter")
    ...
    async def _check_token_bucket(self, redis_key: str, current_time: float) -> RateLimitResult:
    ...
    async def _check_sliding_window(self, redis_key: str, current_time: float) -> RateLimitResult:
...
@asynccontextmanager
async def rate_limit_context(limiter: RateLimiter, 
                            identifier: str,
                            context: Optional[Dict[str, Any]] = None,
                            raise_on_limit: bool = True) -> AsyncContextManager[RateLimitResult]:
...
def get_global_rate_limiter(name: str, config: Optional[RateLimitConfig] = None) -> RateLimiter:
