...
class RedisMessageQueue(BaseMessageQueue):
    ...
    def __init__(self, redis_url: str, config: Dict[str, Any]):
        ...
        self.processing_prefix = config.get('processing_prefix', 'skauswatch:processing')
    ...
    async def publish(self, message: Message) -> bool:
        ...
        try:
            ...
            logger.debug(f"Published message {message.message_id} to {message.destination}")
            return True
    ...
    async def subscribe(self, queue_name: str, handler: MessageHandler) -> None:
        ...
        logger.info(f"Subscribed to queue {queue_name} with handler for {handler.message_type}")
    ...
    async def _process_message(self, 
        ...
        try:
            ...
            await self.redis_client.expire(f"{processing_key}:{message.message_id}", int(handler.timeout))
    ...
    async def nack(self, message_id: str, retry: bool = True) -> bool:
        ...
        try:
            ...
            if retry and message.retry_count < message.max_retries:
                ...
                logger.debug(f"Retrying message {message_id} (attempt {message.retry_count})")
            else:
...
async def initialize_global_message_queue(config: Dict[str, Any]) -> MessageQueueManager:
