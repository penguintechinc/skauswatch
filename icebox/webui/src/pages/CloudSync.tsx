...
export default function CloudSync() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.get<ApiResponse<PaginatedResponse<CloudIntegration>>>(
  ...
  const createMutation = useMutation({
    mutationFn: (payload: CreateIntegrationForm) =>
      apiClient.post('/api/v1/sync/integrations', payload),
    onSuccess: () => {
  ...
  const triggerMutation = useMutation({
    mutationFn: (id: string) =>
      apiClient.post(`/api/v1/sync/integrations/${id}/trigger`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['sync-integrations'] }),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      apiClient.delete(`/api/v1/sync/integrations/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['sync-integrations'] }),
  });
