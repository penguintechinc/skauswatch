...
import type { LicenseInfo, ApiResponse } from '../types/icebox.ts';
...
export default function SettingsPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiClient.get<ApiResponse<LicenseInfo>>('/api/v1/admin/license');
  ...
  const { data: mekData, isLoading: mekLoading, refetch: refetchMek } = useQuery({
    queryKey: ['mek-status'],
    queryFn: async () => {
      const res = await apiClient.get<ApiResponse<MekStatus>>('/api/v1/admin/mek/status');
      return res.data.data!;
    },
  ...
  const licenseMutation = useMutation({
    mutationFn: (key: string) =>
      apiClient.post<ApiResponse<LicenseInfo>>('/api/v1/admin/license', { license_key: key }),
    onSuccess: () => {
      setLicenseKey('');
      refetchLicense();
  ...
  const mekMutation = useMutation({
    mutationFn: () =>
      apiClient.post<ApiResponse<MekRotateResult>>('/api/v1/admin/mek/rotate'),
    onSuccess: (res) => {
            ...
            {licenseLoading ? (
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  {licenseData.validated ? (
                    <CheckCircle size={18} className="text-emerald-400" />
                  ) : (
                    <AlertTriangle size={18} className="text-rose-400" />
                  )}
                  <span
                    className={`font-medium ${licenseData.validated ? 'text-emerald-400' : 'text-rose-400'}`}
                  >
                    {licenseData.validated ? 'Valid License' : 'No Valid License'}
                  </span>
                </div>
