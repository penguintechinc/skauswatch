...
import type { JitRequest, ApiResponse, PaginatedResponse, JitStatus } from '../types/icebox.ts';
...
export default function JitAccess() {
  ...
  const pendingQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.get<ApiResponse<PaginatedResponse<JitRequest>>>(
  ...
  const approveMutation = useMutation({
    mutationFn: ({ id, duration }: { id: string; duration: number }) =>
      apiClient.patch(`/api/v1/jit/requests/${id}/approve`, { approved_duration_seconds: duration }),
    onSuccess: () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: string) => apiClient.patch(`/api/v1/jit/requests/${id}/reject`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['jit-pending'] });
      qc.invalidateQueries({ queryKey: ['jit-mine'] });
    },
