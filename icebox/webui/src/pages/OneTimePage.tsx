...
import type { OneTimeSecret, ApiResponse, PaginatedResponse } from '../types/icebox.ts';
...
export default function OneTimePage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiClient.get<ApiResponse<PaginatedResponse<OneTimeSecret>>>(
  ...
  const createMutation = useMutation({
    mutationFn: (payload: { value: string; ttl_seconds: number }) =>
      apiClient.post<ApiResponse<{ url_token: string; view_url: string; expires_at: string }>>(
        '/api/v1/one-time-secrets',
        payload,
      ),
    onSuccess: (res) => {
