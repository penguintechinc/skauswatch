//! YARA pattern-matching malware detection engine, extracted from
//! `services/scanner/src/yara.rs` (itself ported from v1
//! `scanner/yara_scanner.py`) using yara-x 1.19.0 (pure Rust, no C deps).
//! This was previously the only YARA implementation in the workspace;
//! `services/s3scan` had a `yara_enabled` config flag wired through but
//! never invoked it (see `docs/v2-port/v2.1-depgate.md` §3) — this module
//! is what lets s3scan finally run it too.
...
impl YaraScanner {
    ...
    /// Fails loudly if rules cannot be compiled (unlike v1 which silently
    /// no-op'd).
    ///
    /// # Errors
    /// Returns an error when the path does not exist, no rule files are
    /// found in a directory, or a rule file fails to compile.
    ...
    ///
    /// # Errors
    /// Returns an error when the underlying `yara-x` scan itself fails.
    ...
    ///
    /// # Errors
    /// Returns an error when the file cannot be read or the scan fails.
...
mod tests {
    ...
    async fn test_yara_scanner_with_corporate_threats_corpus() {
        ...
        // two levels up from crates/skauswatch-scan-core just as it is two
        // levels up from services/scanner).
