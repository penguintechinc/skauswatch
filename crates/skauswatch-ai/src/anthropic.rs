...
impl AnthropicProvider {
    ...
    pub fn new(api_key: String) -> anyhow::Result<Self> {
        Self::with_base_url(api_key, "https://api.anthropic.com".to_owned())
    }
...
mod tests {
    ...
    fn new_rejects_an_empty_api_key() {
        assert!(AnthropicProvider::new(String::new()).is_err());
    }
    ...
    async fn complete_returns_content_and_model_on_success() {
        ...
        let provider = AnthropicProvider::with_base_url("sk-ant-test".to_owned(), mock.uri())
    ...
    #[tokio::test]
    async fn complete_maps_a_response_missing_text_to_a_provider_error() {
        let mock = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/v1/messages"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({})))
