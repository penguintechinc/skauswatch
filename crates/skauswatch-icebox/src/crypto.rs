//! IceBox envelope encryption — byte-exact Rust port of the v1 Python
//! `crypto/envelope.py` (`EnvelopeEncryption`).
//!
//! Construction (unchanged from v1 — see `docs/v2-port/icebox-crypto-gate.md`
...
pub enum EnvelopeError {
    /// A required `ICEBOX_MEK_V{n}` (or `ICEBOX_MEK`) env var was not set.
    #[error("{0} (or ICEBOX_MEK) environment variable not set")]
    MissingMek(String),
...
/// Envelope encryption engine for IceBox secrets. Holds every loaded MEK
...
impl EnvelopeEncryption {
    ...
    /// Loads MEK versions from the standard `ICEBOX_MEK*` environment
    /// variables: `ICEBOX_MEK_CURRENT_VERSION` (default `1`), then
    /// `ICEBOX_MEK_V{1..=current}` (falling back to bare `ICEBOX_MEK` for
    /// each), matching v1 `EnvelopeEncryption.from_env`.
    pub fn from_env() -> Result<Self, EnvelopeError> {
        let current_version: u32 = std::env::var("ICEBOX_MEK_CURRENT_VERSION")
            .ok()
        ...
        for v in 1..=current_version {
            let var_name = format!("ICEBOX_MEK_V{v}");
            let mek_b64 = std::env::var(&var_name)
                .ok()
                .or_else(|| std::env::var("ICEBOX_MEK").ok())
