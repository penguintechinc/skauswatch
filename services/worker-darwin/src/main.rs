//! SkausWatch Darwin code review worker entry point. Consumes the
//! `darwin:tasks` stream with the shared consumer-group harness and runs the
...
use skauswatch_streams::{ConsumerConfig, STREAM_DARWIN_TASKS, StreamConsumer, StreamProducer};

use crate::config::WorkerConfig;
use crate::handler::DarwinReviewHandler;

/// SkausWatch Darwin code review worker.
#[derive(Parser)]
#[command(name = "skauswatch-worker-darwin", version)]
struct Cli {
    #[command(subcommand)]
...
async fn serve() -> anyhow::Result<()> {
    skauswatch_telemetry::init_tracing("skauswatch-worker-darwin");
    ...
    let handler = DarwinReviewHandler::new(pool, producer, cfg.clone());
