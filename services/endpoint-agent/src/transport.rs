//! REST client that speaks the manager's `/api/v1/endpoint/*` HMAC-authenticated
...
//!    `services/manager/src/routes/endpoint.rs` require
//!    `X-API-Key = hex(HMAC-SHA256(ENDPOINT_API_SECRET, agent_id))`, computed
...
//!    `EndpointHeartbeatRequest`/`HeartbeatBody` requires `agent_id`, so v1's
//!    heartbeat would 400/500. This client sends `{agent_id, status,
//!    metadata}`.
//! 3. **Event shape.** v1 sent `{type, timestamp, severity, data}` with no
//!    `agent_id` field. The manager's `EndpointEventRequest`/`parse_event`
...
/// `expected_api_key` (services/manager/src/routes/endpoint.rs) and v1 Python's
...
/// `POST /api/v1/endpoint/register` body — field set matches the manager's
...
/// One event in a `POST /api/v1/endpoint/events` batch — matches the fields
/// `parse_event` (services/manager/src/routes/endpoint.rs) reads.
#[derive(Debug, Serialize)]
...
/// Talks the manager's HMAC-authenticated ENDPOINT protocol over HTTPS.
pub struct Reporter {
    client: Client,
    manager_url: String,
    api_secret: String,
...
impl Reporter {
    ...
    /// `POST /api/v1/endpoint/register`. Accepts manager status 200 (re-register)
    /// or 201 (new agent).
    ...
    /// `POST /api/v1/endpoint/heartbeat`. `Ok(false)` signals a 404 ("Agent not
    ...
    /// `POST /api/v1/endpoint/events` for a batch of at most
    ...
    /// `GET /api/v1/endpoint/config`. Implemented for wire-protocol completeness
...
mod tests {
    ...
    /// (services/manager/src/routes/endpoint.rs `hmac_matches_python_hexdigest_vector`):
    /// `hmac.new(b"change-me-endpoint-secret", b"agent-001", sha256).hexdigest()`.
    #[test]
    fn hmac_known_answer_vector_matches_manager() {
        let key = must(
            compute_api_key("change-me-endpoint-secret", "agent-001"),
            "hmac",
        );
        assert_eq!(
            key,
