//! SkausWatch ENDPOINT Agent binary — Rust port of v1's Go `endpoint-agent`
//! (`cmd/endpoint-agent/main.go`). CLI flags match v1's cobra command exactly so
...
use skauswatch_endpoint_agent::agent;
use skauswatch_endpoint_agent::config::{self, CliOverrides};

/// SkausWatch ENDPOINT Agent — monitors system activity, detects security
/// threats, and reports events to the SkausWatch Manager service.
#[derive(Parser, Debug)]
#[command(name = "endpoint-agent", version)]
struct Cli {
    /// Config file (default: /etc/skauswatch/endpoint-agent.yaml, then
    /// ./endpoint-agent.yaml).
    #[arg(long)]
...
fn init_tracing(cfg: &config::AgentConfig) {
    ...
    if !to_stderr && cfg.logging.output != "stdout" && !cfg.logging.output.is_empty() {
        eprintln!(
            "[endpoint-agent] logging.output={:?} is not a supported destination (stdout/stderr only); using stdout",
            cfg.logging.output
        );
    }
