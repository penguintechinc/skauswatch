...
//! `services/manager/src/routes/endpoint.rs` exactly. Complements the pure unit
...
use skauswatch_endpoint_agent::config::TlsConfig;
use skauswatch_endpoint_agent::transport::{
    EventPayload, RegisterRequest, Reporter, compute_api_key,
};
...
async fn register_sends_hmac_header_and_manager_contract_shape() {
    ...
    Mock::given(method("POST"))
        .and(path("/api/v1/endpoint/register"))
...
async fn heartbeat_sends_agent_id_in_body_not_just_headers() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/api/v1/endpoint/heartbeat"))
        .and(header("X-API-Key", expected_api_key().as_str()))
        .and(header("X-Agent-ID", AGENT_ID))
        .and(body_json(
...
async fn report_events_sends_manager_contract_event_shape() {
    let server = MockServer::start().await;
    Mock::given(method("POST"))
        .and(path("/api/v1/endpoint/events"))
        .and(header("X-API-Key", expected_api_key().as_str()))
        .and(body_json(json!([
            {
                "agent_id": AGENT_ID,
...
async fn get_config_parses_manager_response() {
    ...
    Mock::given(method("GET"))
        .and(path("/api/v1/endpoint/config"))
        .and(header("X-Agent-ID", AGENT_ID))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "agent_id": AGENT_ID,
            "config": {
