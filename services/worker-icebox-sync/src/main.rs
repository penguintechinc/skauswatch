//! IceBox cloud-sync worker. `serve` (default) consumes all five provider
//! streams (`icebox:sync:{aws,azure,gcp,oracle,kubernetes}`) with the shared
...
/// IceBox cloud-sync worker.
#[derive(Parser)]
#[command(name = "skauswatch-worker-icebox-sync", version)]
struct Cli {
    #[command(subcommand)]
...
async fn serve() -> anyhow::Result<()> {
    skauswatch_telemetry::init_tracing("skauswatch-worker-icebox-sync");
    ...
    // Same shared `skauswatch` namespace as the icebox REST service (see
    // services/icebox/src/routes/sync.rs::sync_stream_name).
