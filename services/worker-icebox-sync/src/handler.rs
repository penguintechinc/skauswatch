...
//! stream events, `icebox` service's `POST /sync/integrations/{id}/trigger`,
...
/// against that provider's cloud API and updating `icebox_cloud_sync_state`.
...
impl SyncHandler {
    ...
    async fn load_integration(&self, integration_id: &str) -> Option<LoadedIntegration> {
        ...
        let row = sqlx::query_as::<_, Row>(
            "SELECT encrypted_credentials, config FROM icebox_cloud_integrations \
    ...
    async fn remove_sync_state(&self, secret_id: &str, integration_id: &str) {
        let res = sqlx::query(
            "DELETE FROM icebox_cloud_sync_state WHERE secret_id = $1 AND integration_id = $2",
        )
        .bind(secret_id)
        .bind(integration_id)
        .execute(&self.db)
