...
/// Tag applied to every secret IceBox creates, so `list_secrets` can filter
/// to IceBox-managed resources only (matches v1 `ICEBOX_TAG_KEY`/`_VALUE`).
const ICEBOX_TAG_KEY: &str = "icebox:managed";
const ICEBOX_TAG_VALUE: &str = "true";

fn str_field(value: &Value, key: &str) -> Option<String> {
    value.get(key).and_then(Value::as_str).map(str::to_owned)
}

/// Syncs secrets between IceBox and AWS Secrets Manager.
...
impl AwsProvider {
    ...
    /// `secret_prefix` config key defaults to `icebox/`).
    pub fn new(credentials: &Value, config: &Value) -> Self {
        ...
        let prefix = str_field(config, "secret_prefix").unwrap_or_else(|| "icebox/".to_owned());
            ...
            Err(err) => {
                ...
                let create = self
                    ...
                    .tags(
                        Tag::builder()
                            .key(ICEBOX_TAG_KEY)
                            .value(ICEBOX_TAG_VALUE)
                            .build(),
                    )
                    .tags(
                        Tag::builder()
                            .key("icebox:secret_id")
                            .value(secret_id)
                            .build(),
                    )
                    .send()
...
mod tests {
    ...
    fn secret_name_applies_configured_prefix() {
        ...
        assert_eq!(provider.secret_name("db-password"), "icebox/db-password");
    }
