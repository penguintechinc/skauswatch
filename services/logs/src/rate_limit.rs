//! Per-IP rate limiting (`tower_governor`) for the production ingest
//! router — env-overridable via `LOGS_RATE_LIMIT_PER_SECOND` /
//! `LOGS_RATE_LIMIT_BURST` so operators can tune per deployment without a
//! rebuild. Defaults are higher than the other three services in this
//! pass: `POST /ingest` is the manager's SIEM proxy target and log
//! shippers legitimately post in bursts.
//!
...
//! `crate::ingest::router` ITSELF: `tower_governor`'s key extractor needs
...
//! (what `ingest.rs`'s own `test_server`/`test_server_with_license` use)
//! provides neither, so it would fail every request with
//! `GovernorError::UnableToExtractKey` (500) rather than exercising the
//! route under test. Wrapping only in `main.rs` keeps `ingest::router` (and
//! every test built on it) unchanged; `main.rs` also wires
//! `into_make_service_with_connect_info` as a defense-in-depth fallback.
//! This module's own tests below set the forwarded-for header directly
//! instead of needing either of those.
...
/// Default requests/second — `LOGS_RATE_LIMIT_PER_SECOND`.
const DEFAULT_PER_SECOND: u64 = 20;
/// Default burst size — `LOGS_RATE_LIMIT_BURST`.
const DEFAULT_BURST: u32 = 40;

/// Applies the per-IP rate limit to `router`, reading
/// `LOGS_RATE_LIMIT_PER_SECOND`/`LOGS_RATE_LIMIT_BURST` (falling back to
/// [`DEFAULT_PER_SECOND`]/[`DEFAULT_BURST`] when unset, empty, or
/// unparsable).
pub fn apply<S>(router: Router<S>) -> Router<S>
where
    S: Clone + Send + Sync + 'static,
{
    let per_second = env_u64("LOGS_RATE_LIMIT_PER_SECOND", DEFAULT_PER_SECOND);
    let burst = env_u32("LOGS_RATE_LIMIT_BURST", DEFAULT_BURST);
...
mod tests {
    ...
    /// `LOGS_RATE_LIMIT_PER_SECOND`/`LOGS_RATE_LIMIT_BURST` are unset in the
    /// test process, so this also proves the unset -> default fallback path
