...
//! `healthcheck` is the container-native probe; `migrate` applies pending
//! SQL migrations (K8s Job target only — never run at `serve` startup).
//! Rust port of `icebox/services/sync-worker/worker.py`.
...
enum Command {
    ...
    /// `services/worker-vault-sync/migrations` against the configured
    /// database and exits — the K8s Job migration target
    /// (`skauswatch-worker-vault-sync migrate`). Schema authority is
    /// `sqlx migrate`, never an auto-run at `serve` startup (see
...
mod migrate_tests {
    ...
    /// `services/worker-vault-sync/migrations/*.sql` — see
