...
#[derive(Parser)]
#[command(name = "skauswatch-worker-s3", version)]
struct Cli {
    #[command(subcommand)]
...
async fn serve() -> anyhow::Result<()> {
    skauswatch_telemetry::init_tracing("skauswatch-worker-s3");
    ...
    tracing::info!(
        ...
        "starting S3 scan worker (YARA runs in worker-scanner, not worker-s3)"
    );
