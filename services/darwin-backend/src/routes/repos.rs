//! /api/v1/darwin/repos — repository configuration CRUD. Port of
//! `darwin/services/flask-backend/app/api/v1/configs.py`, reshaped onto the
//! `darwin_repo_configs` columns worker-darwin already queries
//! (services/worker-darwin/src/db.rs) and the URL shape the manager proxy
//! already tests (services/manager/src/routes/darwin.rs): `GET/POST /repos`,
...
/// Router for /api/v1/darwin/repos.
...
/// GET /darwin/repos — list repository configurations.
async fn list_repos(
    ...
    let query = format!("SELECT {REPO_CONFIG_COLUMNS} FROM darwin_repo_configs ORDER BY repo_name");
...
async fn create_repo(
    ...
    let existing: Option<(i64,)> =
        sqlx::query_as("SELECT id FROM darwin_repo_configs WHERE provider = $1 AND repo_name = $2")
            .bind(&body.provider)
            .bind(&body.repo_name)
    ...
    let query = format!(
        "INSERT INTO darwin_repo_configs \
...
async fn get_repo(
    ...
    let query = format!("SELECT {REPO_CONFIG_COLUMNS} FROM darwin_repo_configs WHERE id = $1");
...
/// PUT /darwin/repos/{repo_id} body — all fields optional (partial update).
...
/// PUT /darwin/repos/{repo_id} — admin only.
async fn update_repo(
    ...
    let exists: Option<(i64,)> = sqlx::query_as("SELECT id FROM darwin_repo_configs WHERE id = $1")
        .bind(repo_id)
        .fetch_optional(&state.db)
        .await?;
    if exists.is_none() {
    ...
    let mut qb = sqlx::QueryBuilder::<sqlx::Postgres>::new("UPDATE darwin_repo_configs SET ");
    {
...
/// DELETE /darwin/repos/{repo_id} — admin only.
async fn delete_repo(
    ...
    let result = sqlx::query("DELETE FROM darwin_repo_configs WHERE id = $1")
        .bind(repo_id)
        .execute(&state.db)
        .await?;
