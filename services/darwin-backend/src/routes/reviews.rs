//! /api/v1/darwin/reviews — review CRUD + task enqueue. Port of
//! `darwin/services/flask-backend/app/api/v1/reviews.py`. Creating a review
//! enqueues a `DarwinReviewTask` onto the `darwin:tasks` stream
//! (skauswatch_streams::STREAM_DARWIN_TASKS) with the exact field names
//! `services/worker-darwin/src/message.rs` parses.
//!
//! POST is gated to the `maintainer` role only — a deliberate replication of
//! the manager's already-tested proxy gate
//! (`services/manager/src/routes/darwin.rs::create_review`), which admits
...
use skauswatch_streams::STREAM_DARWIN_TASKS;
...
/// Router for /api/v1/darwin/reviews.
...
/// GET /darwin/reviews — paginated, optionally filtered by `repo_config_id`
/// and `status`.
async fn list_reviews(
    ...
    let mut qb = sqlx::QueryBuilder::<sqlx::Postgres>::new(format!(
        "SELECT {REVIEW_COLUMNS} FROM darwin_reviews WHERE 1=1"
    ));
    ...
    let mut count_qb =
        sqlx::QueryBuilder::<sqlx::Postgres>::new("SELECT count(*) FROM darwin_reviews WHERE 1=1");
    if let Some(repo_config_id) = q.repo_config_id {
        count_qb
...
/// Builds the ordered `darwin:tasks` field list — a pure function so the
/// wire contract with worker-darwin (services/worker-darwin/src/message.rs)
...
/// POST /darwin/reviews — maintainer only (see module docs); enqueues onto
/// `darwin:tasks` after the row is committed.
async fn create_review(
    ...
    let repo_config = sqlx::query_as::<_, RepoConfigRef>(
        "SELECT id, tenant_id, provider, repo_name, default_ai_provider \
         FROM darwin_repo_configs WHERE id = $1",
    )
    ...
    if let Some(external_id) = &body.external_id {
        let existing: Option<(i64,)> =
            sqlx::query_as("SELECT id FROM darwin_reviews WHERE external_id = $1")
                .bind(external_id)
                .fetch_optional(&state.db)
                .await?;
        if existing.is_some() {
    ...
    state.publish_stream(STREAM_DARWIN_TASKS, fields).await;
...
/// GET /darwin/reviews/{review_id} — review detail enriched with comments.
async fn get_review(
    State(state): State<AppState>,
    _user: CurrentUser,
    ...
    let query = format!("SELECT {REVIEW_COLUMNS} FROM darwin_reviews WHERE id = $1");
    ...
    let comments = sqlx::query_as::<_, ReviewComment>(
        "SELECT id, file_path, line_number, comment, category, severity, created_at \
         FROM darwin_review_comments WHERE review_id = $1 ORDER BY created_at",
    )
    .bind(review_id)
    .fetch_all(&state.db)
    .await?;
...
mod tests {
    ...
    #[test]
    fn task_fields_match_worker_darwin_message_contract() {
