//! GET /api/v1/darwin/status — service status, proxied verbatim by the
//! manager. Port of the v1 `/api/v1/darwin/status` shape.
...
/// Router for GET /darwin/status.
pub fn router() -> Router<AppState> {
    Router::new().route("/darwin/status", get(darwin_status))
}

/// GET /darwin/status — `{status, queue_depth}`, where `queue_depth` is the
...
async fn darwin_status(
    ...
    let queue_depth: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM darwin_reviews WHERE status IN ('queued', 'in_progress', 'processing')",
    )
    .fetch_one(&state.db)
    .await?;
...
mod tests {
    ...
    async fn status_requires_auth() {
        ...
        let resp = server.get("/api/v1/darwin/status").await;
    ...
    async fn status_denied_without_license_even_with_valid_token() {
        ...
        let body: serde_json::Value = resp.json();
        assert_eq!(body["error"], "Darwin AI review requires a Darwin license.");
    }
}
