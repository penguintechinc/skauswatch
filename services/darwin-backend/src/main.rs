//! SkausWatch Darwin AI-code-review backend entry point. `serve` (default)
//! runs the `/api/v1/darwin` + `/api/v1/credentials` REST surface and the
...
/// SkausWatch Darwin backend service.
#[derive(Parser)]
#[command(name = "skauswatch-darwin-backend", version)]
struct Cli {
    #[command(subcommand)]
...
/// Default REST port for the Darwin backend (env `API_PORT`). Matches the
/// `WORKER_DARWIN_URL` default port the manager proxies to
/// (services/manager/src/routes/darwin.rs).
...
async fn serve() -> anyhow::Result<()> {
    skauswatch_telemetry::init_tracing("skauswatch-darwin-backend");
    ...
    tracing::info!(%addr, "darwin-backend REST listening");
    readiness.set_ready();
    ...
    axum::serve(listener, app)
        ...
        .await?;
    tracing::info!("darwin-backend stopped cleanly");
    Ok(())
}
