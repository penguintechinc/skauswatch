...
//! producer used to enqueue review tasks onto `darwin:tasks`.
...
pub struct AppStateInner {
    ...
    /// Postgres pool (per-service `darwin` account, `darwin_*` tables only).
    pub db: PgPool,
    /// JWT auth settings.
    pub auth: AuthSettings,
    /// AES-256-GCM cipher for git credential tokens at rest.
    pub crypto: Arc<CredentialCipher>,
    /// Redis Streams producer for `darwin:tasks`. `None` only in tests —
