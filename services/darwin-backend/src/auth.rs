//! AuthN/Z for the Darwin backend. Tokens are issued centrally by the
...
//! identity table (see migrations/0001_darwin_schema.sql), so `CurrentUser`
...
//! check on `/api/v1/darwin/*` (services/manager/src/routes/darwin.rs).
