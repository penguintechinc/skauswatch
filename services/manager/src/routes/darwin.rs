//! /api/v1/darwin — authenticated pure proxy to the Darwin AI-code-review
//! worker (`WORKER_DARWIN_URL`), gated on the `skauswatch.darwin` module
//! flag. Contract: docs/v2-port/manager-contract.md §darwin; Python source
//! of truth: services/manager/api/v1/darwin.py.
...
/// PostHog module flag gating every darwin route (see `crate::flags`
/// `MODULE_FLAGS`) — the v2 equivalent of v1 `has_feature("darwin")`.
const DARWIN_FLAG: &str = "skauswatch.darwin";
...
/// Router for /api/v1/darwin — a pure proxy to the worker-darwin backend.
pub fn router() -> Router<AppState> {
    Router::new()
        .route("/darwin/status", get(darwin_status))
...
/// Shared HTTP client for worker-darwin proxying — built once per process
...
/// v1 `DARWIN_BASE`: `{WORKER_DARWIN_URL}/api/v1/darwin`, default
/// `http://worker-darwin:5005`. Read per request (house pattern: siem.rs).
fn darwin_base() -> String {
    let url = std::env::var("WORKER_DARWIN_URL")
        .unwrap_or_else(|_| "http://worker-darwin:5005".to_owned());
    format!("{url}/api/v1/darwin")
}

/// v1 `_check_darwin_license` equivalent: the bare v1 403 body when the
/// `skauswatch.darwin` flag is off. penguin-licensing is fail-safe and never
...
fn transport_error(e: &reqwest::Error) -> Response {
    let (status, msg) = if e.is_timeout() {
        (
            ...
            "worker-darwin timeout".to_owned(),
        )
    } else if e.is_connect() {
        (
            StatusCode::SERVICE_UNAVAILABLE,
            "Cannot connect to worker-darwin".to_owned(),
        )
    } else {
...
/// Forwards one request to worker-darwin and shapes the reply exactly like
...
/// Proxies one manager route to `{WORKER_DARWIN_URL}/api/v1/darwin{path}`
...
/// GET /darwin/status — Darwin service status, proxied verbatim.
async fn darwin_status(
...
/// admins get 403 (contract §darwin: "reviews (POST maintainer)").
...
mod tests {
    ...
    /// offline (community fallback + empty flag set), so `skauswatch.darwin`
    ...
    /// Boots a TestServer with only the darwin router nested under /api/v1 —
    ...
    async fn license_gate_denies_with_v1_body_when_flag_off() {
        ...
        let denied = match license_denied(&state).await {
            Some(d) => d,
            None => panic!("expected the darwin gate to deny"),
        };
        let (status, body) = response_parts(denied).await;
        assert_eq!(status, StatusCode::FORBIDDEN);
        assert_eq!(
            body,
            serde_json::json!({"error": "Darwin AI review requires a Darwin license."})
        );
    }
    ...
    async fn proxy_forwards_auth_header_and_passes_json_through() {
        ...
        let resp = forward(
            &format!("{}/api/v1/darwin", upstream.uri()),
    ...
    async fn proxy_forwards_query_params_and_json_body() {
        ...
        let base = format!("{}/api/v1/darwin", upstream.uri());
        let listed = forward(
            &base,
            Duration::from_secs(5),
            reqwest::Method::GET,
    ...
    async fn proxy_passes_upstream_error_status_and_body_through() {
        let upstream = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/api/v1/darwin/reviews"))
            .respond_with(
    ...
    async fn proxy_timeout_maps_to_v1_504() {
        ...
        assert_eq!(body, serde_json::json!({"error": "worker-darwin timeout"}));
    }
    ...
    async fn proxy_unreachable_upstream_maps_to_v1_503() {
        ...
        let resp = forward(
            "http://127.0.0.1:1/api/v1/darwin",
        ...
        assert_eq!(
            body,
            serde_json::json!({"error": "Cannot connect to worker-darwin"})
        );
    }
