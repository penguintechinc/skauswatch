    ...
    def generate_qr_code(self, user_email: str, secret: str) -> str:
        ...
        try:
            totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(
                name=user_email, issuer_name=self.config.mfa_issuer
            )
            ...
            session_record = (
                ...
                .select(
                    ...
                    left=self.db.auth_user.on(
                        self.db.auth_user.id == self.db.auth_session.user_id
                    ),
                )
                .first()
            )
    ...
    def user_has_permission(self, user_id: int, resource: str, action: str) -> bool:
        ...
        try:
            ...
            permission_query = self.db(
                ...
                & (
                    self.db.auth_role_permission.permission_id
                    == self.db.auth_permission.id
                )
...
class SkausWatchAuth(Auth):
    ...
    def login(
        ...
        try:
            ...
            if (
                ...
                and user.account_locked_until > datetime.datetime.utcnow()
            ):
                self._log_security_event(
                    "authentication_failure",
            ...
            if user.mfa_enabled:
                ...
                if user.mfa_secret and self.mfa_manager.verify_token(
    ...
    def change_password(
        ...
        try:
            ...
            password_valid, password_errors = self.password_policy.validate(
                new_password
            )
            if not password_valid:
            ...
            # Hash new password
            password_hash = str(
                Field("password", "password", requires=[])(new_password)[0]
            )

            # Update password
            self.db(self.db.auth_user.id == user_id).update(
                password=password_hash, password_changed_at=datetime.datetime.utcnow()
            )
            self.db.commit()
    ...
    def verify_mfa_setup(self, user_id: int, token: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            # Enable MFA
            self.db(self.db.auth_user.id == user_id).update(mfa_enabled=True)
            self.db.commit()
    ...
    def disable_mfa(self, user_id: int) -> Tuple[bool, str]:
        ...
        try:
            # Update user
            self.db(self.db.auth_user.id == user_id).update(
                mfa_enabled=False, mfa_secret=None, backup_codes=None
            )
            self.db.commit()
