...
class AsyncDatabaseManager:
    ...
    async def initialize(self):
        ...
        health_config = HealthCheckConfig(
            enabled=True, interval=30.0, timeout=5.0, query="SELECT 1"
        )

        await self.pool_manager.create_database_pool(
            "manager_db", self.config.url, pool_config, health_config
        )
        ...
        if self.config.enable_caching:
            ...
            cache_config = CacheConfig(
                max_size=1000, default_ttl=self.config.cache_ttl, metrics_enabled=True
            )
    ...
    async def get_user_by_id(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        result = await self.execute_query(
            query, {"user_id": user_id}, fetch="one", cache_key=f"user:{user_id}"
        )
    ...
    async def get_user_permissions(self, user_id: str) -> List[str]:
        ...
        """
        result = await self.execute_query(
            query, {"user_id": user_id}, cache_key=f"permissions:{user_id}"
        )
    ...
    async def update_approval_status(
        self, approval_id: str, status: str, user_id: str, comment: Optional[str] = None
    ) -> bool:
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        user_stats = await self.execute_query(
            "SELECT COUNT(*) as total_users FROM users", fetch="one"
        )
            ...
            for row in table_sizes[:5]:  # Top 5 largest tables
                ...
                try:
                    await self.execute_query(
                        f"VACUUM ANALYZE {table_name}", fetch="none"
                    )
...
class AsyncBackgroundProcessor:
    ...
    async def _cleanup_audit_logs(self, days_to_keep: int = 90):
        ...
        result = await self.db_manager.execute_query(
            query, {"days": days_to_keep}, fetch="none"
        )
    ...
    async def _send_notification(
        self, user_id: str, message: str, notification_type: str = "info"
    ):
