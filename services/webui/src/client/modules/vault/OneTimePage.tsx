...
import type { OneTimeSecret, ApiResponse, PaginatedResponse } from '../../types/vault';
...
export default function OneTimePage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiVault.get<ApiResponse<PaginatedResponse<OneTimeSecret>>>(
  ...
  const createMutation = useMutation({
    mutationFn: (payload: { value: string; ttl_seconds: number }) =>
      apiVault.post<ApiResponse<{ url_token: string; view_url: string; expires_at: string }>>(
        '/api/v1/one-time-secrets',
        payload,
      ),
    onSuccess: (res) => {
