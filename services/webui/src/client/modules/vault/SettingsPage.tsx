...
import type { LicenseInfo, ApiResponse } from '../../types/vault';
...
export default function SettingsPage() {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiVault.get<ApiResponse<LicenseInfo>>('/api/v1/admin/license');
  ...
  const { data: mekData, isLoading: mekLoading, refetch: refetchMek } = useQuery({
    queryKey: ['mek-status'],
    queryFn: async () => {
      const res = await apiVault.get<ApiResponse<MekStatus>>('/api/v1/admin/mek/status');
      return res.data.data!;
    },
  ...
  const licenseMutation = useMutation({
    mutationFn: (key: string) =>
      apiVault.post<ApiResponse<LicenseInfo>>('/api/v1/admin/license', { license_key: key }),
    onSuccess: () => {
      setLicenseKey('');
      refetchLicense();
  ...
  const mekMutation = useMutation({
    mutationFn: () =>
      apiVault.post<ApiResponse<MekRotateResult>>('/api/v1/admin/mek/rotate'),
    onSuccess: (res) => {
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-sm text-amber-200">
                  ...
                  <ul className="list-disc list-inside space-y-1 text-amber-300/80">
                    <li>Update <code className="font-mono">VAULT_MEK</code> in all pod environments first</li>
