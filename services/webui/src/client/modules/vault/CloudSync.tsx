...
const DIRECTION_LABELS: Record<SyncDirection, string> = {
  vault_to_cloud: 'Vault → Cloud',
  cloud_to_vault: 'Cloud → Vault',
...
export default function CloudSync() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiVault.get<ApiResponse<PaginatedResponse<CloudIntegration>>>(
  ...
  const createMutation = useMutation({
    mutationFn: (payload: CreateIntegrationForm) =>
      apiVault.post('/api/v1/sync/integrations', payload),
    onSuccess: () => {
  ...
  const triggerMutation = useMutation({
    mutationFn: (id: string) =>
      apiVault.post(`/api/v1/sync/integrations/${id}/trigger`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['sync-integrations'] }),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) =>
      apiVault.delete(`/api/v1/sync/integrations/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['sync-integrations'] }),
  });
