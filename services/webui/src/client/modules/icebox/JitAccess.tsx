...
import type { JitRequest, ApiResponse, PaginatedResponse, JitStatus } from '../../types/icebox';
...
export default function JitAccess() {
  ...
  const pendingQuery = useQuery({
    ...
    queryFn: async () => {
      const res = await apiIceBox.get<ApiResponse<PaginatedResponse<JitRequest>>>(
  ...
  const approveMutation = useMutation({
    mutationFn: ({ id, duration }: { id: string; duration: number }) =>
      apiIceBox.patch(`/api/v1/jit/requests/${id}/approve`, { approved_duration_seconds: duration }),
    onSuccess: () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: (id: string) => apiIceBox.patch(`/api/v1/jit/requests/${id}/reject`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['jit-pending'] });
      qc.invalidateQueries({ queryKey: ['jit-mine'] });
    },
