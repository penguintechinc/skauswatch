...
import type { Secret, ApiResponse, PaginatedResponse, SecretType } from '../../types/icebox';
...
export default function Secrets() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['secrets'],
    queryFn: async () => {
      const res = await apiIceBox.get<ApiResponse<PaginatedResponse<Secret>>>('/api/v1/secrets');
      return res.data.data!;
    },
  });

  const createMutation = useMutation({
    mutationFn: (payload: CreateSecretForm) => apiIceBox.post('/api/v1/secrets', payload),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiIceBox.delete(`/api/v1/secrets/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['secrets'] }),
  });

  const rotateMutation = useMutation({
    mutationFn: ({ id, value }: { id: string; value: string }) =>
      apiIceBox.post(`/api/v1/secrets/${id}/rotate`, { new_value: value }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['secrets'] }),
  });

  const handleViewValue = async (id: string) => {
    const res = await apiIceBox.get<ApiResponse<{ value: string }>>(`/api/v1/secrets/${id}/value`);
