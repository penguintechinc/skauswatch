...
import type { DarwinPlan, DarwinRepo, DarwinPlanCreateRequest } from '../../types/darwin';
...
export default function PlansTab() {
  const [plans, setPlans] = useState<DarwinPlan[]>([]);
  ...
  const [form, setForm] = useState<DarwinPlanCreateRequest>({
    repo_config_id: 0,
  ...
  const handleQueue = async () => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      await darwinApi.createPlan(form);
      setShowQueue(false);
  ...
  const loadExpanded = async (id: number) => {
    ...
    try {
      const full = await darwinApi.getPlan(id);
      ...
      {showQueue && (
        <div className="bg-dark-800 border border-dark-700 rounded p-4 space-y-3">
          ...
          <p className="text-dark-400 text-xs">
            Darwin will analyze the issue and generate an AI-powered implementation plan.
          </p>
