...
import { darwinApi } from '../../api/darwin';
import type { DarwinRepo, DarwinRepoCreateRequest } from '../../types/darwin';

export default function ReposTab() {
  const [repos, setRepos] = useState<DarwinRepo[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showAdd, setShowAdd] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [form, setForm] = useState<DarwinRepoCreateRequest>({
    provider: 'github',
    repo_url: '',
    repo_name: '',
    webhook_secret: '',
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await darwinApi.listRepos();
      setRepos(data);
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : 'Failed to load repositories';
      if (msg.includes('403') || msg.includes('license')) {
        setError('Darwin AI Review requires a Darwin license. Contact your administrator.');
      } else {
        setError(msg);
      }
    } finally {
  ...
  const handleAdd = async () => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      await darwinApi.createRepo(form);
      setShowAdd(false);
      ...
      if (msg.includes('limit') || msg.includes('cap') || msg.includes('403')) {
        setError('Repository limit reached (3 repos on community plan). Upgrade to a Darwin license for unlimited repos.');
      } else {
        setError(msg);
      }
    } finally {
  ...
  const toggleActive = async (repo: DarwinRepo) => {
    try {
      await darwinApi.updateRepo(repo.id, { is_active: !repo.is_active });
      await load();
    } catch (e: unknown) {
    ...
    <div className="space-y-4">
      ...
      {repos.length >= 3 && (
        <div className="bg-yellow-900/30 border border-yellow-700 rounded px-4 py-3 text-yellow-300 text-sm">
          Community limit: 3 repositories. Purchase a Darwin license to add more.
        </div>
      )}
