...
import type { DarwinReview, DarwinRepo, DarwinReviewCreateRequest } from '../../types/darwin';
...
export default function ReviewsTab() {
  const [reviews, setReviews] = useState<DarwinReview[]>([]);
  ...
  const [form, setForm] = useState<DarwinReviewCreateRequest>({
    repo_config_id: 0,
    pr_url: '',
    pr_number: undefined,
  });
  ...
  const handleQueue = async () => {
    ...
    try {
      setSubmitting(true);
      setError(null);
      await darwinApi.createReview(form);
      setShowQueue(false);
      ...
      if (msg.includes('limit') || msg.includes('cap') || msg.includes('403')) {
        setError('Daily review limit reached (10/day on community plan). Upgrade to a Darwin license for unlimited reviews.');
      } else {
        setError(msg);
      }
    } finally {
  ...
  const loadExpanded = async (id: number) => {
    ...
    try {
      const full = await darwinApi.getReview(id);
