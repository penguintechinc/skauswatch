...
from ...shared.performance import (
    ...
    ThreadPoolManager, TaskType, CPUBoundTaskManager, IOBoundTaskManager,
...
class AsyncSSHProcessor:
        ...
        try:
            ...
            if ca_key_path:
                ...
                with open(ca_key_path, 'r') as f:
                    self.ca_private_key = paramiko.RSAKey.from_private_key_file(ca_key_path)
            else:
    ...
    def _setup_rate_limiters(self):
        """Setup rate limiters"""
        self.rate_limiters['certificate_signing'] = RateLimiter(RateLimitConfig(
        ...
        self.rate_limiters['config_generation'] = RateLimiter(RateLimitConfig(
    ...
    async def submit_signing_request(self, request: SSHCertificateRequest) -> str:
        ...
        # Check rate limit
        if not await self.rate_limiters['certificate_signing'].is_allowed(request.requester_id):
    ...
    async def submit_revocation_request(self, 
                                      certificate_id: str,
                                      serial_number: int,
                                      reason: str,
                                      revoked_by: str) -> bool:
    ...
    async def submit_config_request(self, 
                                   config_type: str,
                                   user_id: str,
    ...
    async def _process_signing_request(self, 
        ...
        try:
            ...
            logger.error(f"Failed to process SSH signing request {request.request_id}: {e}")
            self.metrics.error_count += 1
            raise
            
    @async_timeout(30.0)
    async def _sign_certificate_async(self, request: SSHCertificateRequest) -> Dict[str, Any]:
        """Sign SSH certificate asynchronously"""
        return await self.cpu_manager.submit_computation(
            self._sign_certificate_sync,
            request
        )
    ...
    def _sign_certificate_sync(self, request: SSHCertificateRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            cert_data = self._build_ssh_certificate(
                ...
                serial_number=serial_number,
                cert_type=1 if request.certificate_type == SSHCertificateType.USER else 2,
    ...
    def _build_ssh_certificate(self,
                              key_type: str,
    ...
    async def _process_revocation(self, revocation_data: Dict[str, Any]):
        ...
        krl_entry = KRLEntry(
            ...
            certificate_fingerprint=self.certificates[certificate_id].get('public_key_fingerprint')
        )
        ...
        self.certificates[certificate_id]['revoked_at'] = revocation_data['revocation_time'].isoformat()
        ...
        while self.running:
            try:
                ...
                krl_request = await asyncio.wait_for(
                    self.krl_queue.get(),
                    timeout=1800.0  # Update KRL every 30 minutes
                )
    ...
    async def _generate_krl(self):
        ...
        krl_data = await self.cpu_manager.submit_computation(
            self._generate_krl_sync
        )
    ...
    async def _process_config_request(self, 
    ...
    async def list_certificates(self,
        ...
        for cert_data in self.certificates.values():
            # Apply filters
            if certificate_type and cert_data.get('certificate_type') != certificate_type.value:
                continue
            if status and cert_data.get('status') != status.value:
                continue
                
            # Remove sensitive data
            safe_cert_data = {k: v for k, v in cert_data.items() if 'private' not in k.lower()}
