...
class DatabaseCollectorConfig:
    ...
    queries: List[Dict[str, Any]] = field(default_factory=list)  # SQL queries to execute
...
class HealthCheckConfig:
    ...
    checks: List[str] = field(default_factory=lambda: [
        "database", "redis", "collectors", "threat_intel", "ai_providers"
    ])
...
class AAAMonitorConfig:
    ...
    def _load_configuration(self):
        ...
        try:
            ...
            self.collectors = CollectorsConfig(
                ...
                database=DatabaseCollectorConfig(**collectors_config.get('database', {}))
            )
            
            # Processing
            self.log_processing = LogProcessingConfig(**config_dict.get('log_processing', {}))
            self.classification = ClassificationConfig(**config_dict.get('classification', {}))
            ...
            self.ai = AIConfig(
                **{k: v for k, v in ai_config.items() if k not in ['openai', 'anthropic', 'ollama']},
            ...
            self.pattern_detection = PatternDetectionConfig(**config_dict.get('pattern_detection', {}))
            self.anomaly_detection = AnomalyDetectionConfig(**config_dict.get('anomaly_detection', {}))
    ...
    def _load_config_file(self, config_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Failed to load configuration file", path=config_path, error=str(e))
            raise
        ...
        return {
            ...
            'ai': {
                **{k: v for k, v in self.ai.__dict__.items() 
                ...
                'openai': {k: v for k, v in self.ai.openai.__dict__.items() if k != 'api_key'},
                'anthropic': {k: v for k, v in self.ai.anthropic.__dict__.items() if k != 'api_key'},
