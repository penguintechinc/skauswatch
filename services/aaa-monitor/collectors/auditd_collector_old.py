...
class AuditdCollector:
    ...
    async def _check_audit_rules(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                rules_count = len(rules_output.strip().split('\n')) if rules_output.strip() else 0
        ...
        for log_file in log_files:
            try:
                ...
                if log_path.exists():
                    ...
                    logger.info("Initialized log position", file=log_file, position=file_size)
                    
            except Exception as e:
                logger.error("Error initializing log position", file=log_file, error=str(e))
    ...
    async def _parse_ssh_auth(self, line: str, timestamp: datetime) -> Optional[AuthenticationEvent]:
    ...
    async def _parse_sudo_event(self, line: str, timestamp: datetime) -> Optional[BaseEvent]:
        """Parse sudo events"""
        try:
            if 'COMMAND' in line:
    ...
    async def _parse_login_event(self, line: str, timestamp: datetime) -> Optional[AuthenticationEvent]:
        ...
        try:
            while self.running:
                try:
                    ...
                    result = await asyncio.create_subprocess_exec(
                        'ausearch', '-ts', 'recent', '--format', 'text',
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE
                    )
                    stdout, stderr = await result.communicate()
                    ...
                    logger.info("ausearch not available, disabling real-time monitoring")
                    break
                except Exception as e:
