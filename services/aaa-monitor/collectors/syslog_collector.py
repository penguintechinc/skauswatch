...
class SyslogCollector:
    ...
    async def initialize(self):
        """Initialize syslog collector"""
        try:
            logger.info("Initializing syslog collector",
    ...
    async def _start_tcp_server(self):
        ...
        try:
            ...
            logger.info("TCP syslog server started",
        ...
        try:
            while self.running:
                ...
                try:
                    ...
                    if message:
                        await self._process_syslog_message(message, 'tcp', client_address)
                        
                except asyncio.TimeoutError:
                    # Keep connection alive
                    continue
                except Exception as e:
                    logger.debug("Error reading from TCP client", 
                               client=client_address, error=str(e))
                    break
                    
        except Exception as e:
            logger.error("Error handling TCP client", client=client_address, error=str(e))
        finally:
            writer.close()
    ...
    async def _connect_to_server(self, server_config):
        ...
        try:
            logger.info("Connecting to syslog server", 
            ...
            while self.running:
                try:
                    if server_config.protocol.lower() == 'tcp':
                        ...
                        while self.running:
                            try:
                                ...
                                logger.debug("Error reading from syslog server", 
                    ...
                    logger.error("Error connecting to syslog server",
                               session_id=session_id, error=str(e))
            ...
            logger.error("Fatal error in syslog server connection", 
                        session_id=session_id, error=str(e))
    ...
    def _parse_rfc3164_timestamp(self, timestamp_str: str) -> datetime:
        ...
        try:
            ...
            timestamp = datetime.strptime(f"{current_year} {timestamp_str}", 
                                        "%Y %b %d %H:%M:%S")
            return timestamp
        except Exception:
    ...
    async def _classify_syslog_message(self, content: str, timestamp: datetime,
                                     hostname: str, tag: str, facility: int, 
                                     severity: int) -> List[BaseEvent]:
        ...
        try:
            ...
            if facility in [4, 10]:  # auth, authpriv
                if any(keyword in content_lower for keyword in 
                       ['login', 'authentication', 'password', 'failed', 'accepted']):
            ...
            # Network events
                    ...
                    ['connection', 'network', 'firewall', 'iptables']):
                event = BaseEvent(
                    source=LogSource.SYSTEM,
    ...
    async def _create_auth_event_from_syslog(self, content: str, timestamp: datetime,
                                           hostname: str, tag: str, 
        ...
        try:
            ...
            # Determine success
            if any(word in content.lower() for word in ['failed', 'failure', 'invalid', 'denied']):
                success = False
    ...
    async def _create_ssh_event_from_syslog(self, content: str, timestamp: datetime,
                                          hostname: str, tag: str,
                                          severity: Severity) -> Optional[AuthenticationEvent]:
    ...
    async def _create_sudo_event_from_syslog(self, content: str, timestamp: datetime,
                                           hostname: str, tag: str,
                                           severity: Severity) -> Optional[BaseEvent]:
