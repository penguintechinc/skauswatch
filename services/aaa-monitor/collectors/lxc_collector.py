...
from ..models import (
    ...
    ProcessEvent, NetworkEvent, FileAccessEvent,
...
class LXCCollector:
    ...
    def __init__(self, config: Dict[str, Any], log_processor, analysis_engine):
        ...
        self.auth_patterns = {
            ...
            'login_failure': re.compile(r'login.*authentication failure', re.IGNORECASE),
        ...
        self.container_patterns = {
            'started': re.compile(r'Started container|Container.*started', re.IGNORECASE),
            'stopped': re.compile(r'Stopped container|Container.*stopped', re.IGNORECASE),
            'created': re.compile(r'Created container|Container.*created', re.IGNORECASE),
            'deleted': re.compile(r'Deleted container|Container.*deleted', re.IGNORECASE),
            ...
            'network_error': re.compile(r'network.*error|network.*failed', re.IGNORECASE),
        }
        
        self.security_patterns = {
            'privilege_escalation': re.compile(r'setuid|setgid|su -|sudo', re.IGNORECASE),
            'file_access_denied': re.compile(r'permission denied|access denied', re.IGNORECASE),
            'network_violation': re.compile(r'connection.*refused|port.*blocked', re.IGNORECASE),
            'resource_limit': re.compile(r'resource.*limit|quota.*exceeded', re.IGNORECASE),
            'capability_violation': re.compile(r'capability.*denied|cap_.*denied', re.IGNORECASE),
        }
        ...
        try:
            ...
            for i, endpoint_config in enumerate(self.config.lxd_endpoints):
                ...
                if endpoint_config.cert_file and endpoint_config.key_file:
                    ssl_context.load_cert_chain(endpoint_config.cert_file, endpoint_config.key_file)
            ...
            logger.info("LXC/LXD collector initialized successfully",
        ...
        try:
            ...
            async with session.get(api_url) as response:
                if response.status == 200:
                    ...
                    logger.info("LXD API connection successful",
                              server=base_url,
            ...
            logger.error("LXD API connection test failed", 
    ...
    async def _update_containers_from_endpoint(self, session_id: str):
        ...
        try:
            ...
            async with session.get(containers_url) as response:
                if response.status != 200:
                    logger.warning("Failed to get containers", 
                                 session_id=session_id, status=response.status)
                    return
                ...
                if added:
                    logger.info("New containers detected", 
                              session_id=session_id, containers=list(added))
                if removed:
                    logger.info("Containers removed", 
                              session_id=session_id, containers=list(removed))
            
        except Exception as e:
            logger.error("Error updating containers from endpoint", 
                        session_id=session_id, error=str(e))
    ...
    async def _get_container_details(self, session_id: str, container_name: str):
        ...
        try:
            ...
            logger.debug("Error getting container details", 
                        session_id=session_id, container=container_name, error=str(e))
    ...
    async def _collect_container_logs_api(self):
        ...
        try:
            while self.running:
                try:
                    for container_key, container_info in self.monitored_containers.copy().items():
                        if not self.running:
                            break
                        
                        await self._process_container_logs_api(
                            container_info['session_id'],
                            container_info['name']
                        )
    ...
    async def _process_container_logs_api(self, session_id: str, container_name: str):
        ...
        try:
            ...
            async with session.get(logs_url) as response:
                if response.status != 200:
                    logger.debug("Container logs not available", 
                               container=container_name, status=response.status)
                    return
            ...
            logger.error("Error processing container logs via API", 
                        container=container_name, error=str(e))
    
    async def _fetch_container_log_file(self, session_id: str, container_name: str, log_file: str):
        ...
        try:
            ...
            log_content_url = urljoin(base_url, f'/1.0/containers/{container_name}/logs/{log_file}')
            ...
            logger.debug("Error fetching container log file", 
                        container=container_name, log_file=log_file, error=str(e))





    async def _parse_container_logs(self, logs_content: str, container_name: str, source_type: str):
        ...
        try:
            ...
            for line in lines:
                ...
                events = await self._classify_and_create_events(line, container_name, timestamp)
            ...
            logger.error("Error parsing container logs", container=container_name, error=str(e))

    async def _classify_and_create_events(self, line: str, container_name: str, timestamp: datetime) -> List[BaseEvent]:
        ...
        try:
            ...
            for pattern_name, pattern in self.auth_patterns.items():
                if pattern.search(line):
                    event = await self._create_auth_event(line, container_name, timestamp, pattern_name)
                    events.append(event)
                    break
            ...
            for pattern_name, pattern in self.container_patterns.items():
                if pattern.search(line):
                    event = await self._create_container_event(line, container_name, timestamp, pattern_name)
                    events.append(event)
                    break
            ...
            for pattern_name, pattern in self.security_patterns.items():
                if pattern.search(line):
                    event = await self._create_security_event(line, container_name, timestamp, pattern_name)
                    events.append(event)
                    break
            
            # If no specific pattern matched, create generic event for significant entries
            if not events and any(word in line_lower for word in ['error', 'warning', 'failed', 'denied']):
                event = BaseEvent(
                    source=LogSource.LXC_LXD,
                    event_type=EventType.SYSTEM_CALL,
                    severity=Severity.MEDIUM if 'warning' in line_lower else Severity.HIGH,
                    message=line,
                    timestamp=timestamp,
                    raw_data={
    ...
    async def _create_auth_event(self, line: str, container_name: str, 
                                timestamp: datetime, pattern_name: str) -> AuthenticationEvent:
    ...
    async def _create_container_event(self, line: str, container_name: str,
                                     timestamp: datetime, pattern_name: str) -> ContainerEvent:
    ...
    async def _create_security_event(self, line: str, container_name: str,
                                    timestamp: datetime, pattern_name: str) -> BaseEvent:
            ...
            for service in services:
                try:
                    ...
                    if result.returncode == 0:
                        await self._parse_container_logs(stdout.decode(), service, 'journalctl')
                        
                except Exception as e:
                    logger.debug("Error getting systemd logs for service", 
    ...
    async def _poll_container_events(self, session_id: str):
        ...
        try:
            ...
            logger.error("Error polling container events", session_id=session_id, error=str(e))
        ...
        try:
            ...
            try:
                stdout, stderr = await asyncio.wait_for(
                    process.communicate(), 
                    timeout=self.config.get('event_timeout', 30)
                )
    ...
    async def _monitor_websocket_events(self, session_id: str):
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    async with websockets.connect(
                        ...
                        logger.info("WebSocket connected for events", session_id=session_id)
                        ...
                        async for message in websocket:
                            ...
                            try:
                                ...
                                await self._process_websocket_event(event_data, session_id)
                            except json.JSONDecodeError:
                    ...
                    logger.error("WebSocket error, retrying...", 
                               session_id=session_id, error=str(e))
            ...
            logger.error("Fatal error in WebSocket events monitoring", 
                        session_id=session_id, error=str(e))
        finally:
    ...
    async def _create_state_change_event(self, session_id: str, container_name: str,
    ...
    async def _create_lxd_lifecycle_event(self, event_data: Dict, session_id: str):
        ...
        try:
            ...
            metadata = event_data.get('metadata', {})
            container_name = metadata.get('name', metadata.get('description', 'unknown'))
    ...
    async def _collect_container_metrics_api(self, session_id: str, container_name: str):
        ...
        try:
            ...
            logger.error("Error collecting container metrics via API", 
                        container=container_name, error=str(e))
    
    async def _process_container_metrics_api(self, metrics_data: Dict, 
        ...
        try:
            ...
            if memory_usage > 0:
                ...
                logger.debug("Container resource usage", 
            ...
            logger.error("Error processing container metrics", 
                        container=container_name, error=str(e))

    async def _process_container_metrics(self, metrics_output: str, container_name: str):
        """Process container metrics and generate events for anomalies"""
        try:
