...
class DatabaseCollector:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info("Database collector initialized successfully",
    ...
    async def _initialize_postgresql(self, conn_id: str, config: Dict):
        ...
        try:
            dsn = f"host={config['host']} port={config.get('port', 5432)} " \
    ...
    async def _execute_query_loop(self, query_config: Dict):
        ...
        try:
            ...
            logger.error("Fatal error in query execution", 
                        query_id=query_id, error=str(e))
    ...
    async def _execute_query(self, query_config: Dict):
        ...
        try:
            ...
            if rows:
                ...
                logger.debug("Processed query results", 
                           query_id=query_id, rows=len(rows))
                
        except Exception as e:
            logger.error("Error executing query", 
                        query_id=query_config['id'], error=str(e))
    ...
    async def _add_time_filter(self, sql: str, query_id: str) -> str:
        ...
        try:
            ...
            last_time = self.query_cursors.get(query_id, 
    ...
    async def _execute_postgresql_query(self, db_info: Dict, sql: str, query_config: Dict) -> List[Dict]:
    ...
    async def _execute_mysql_query(self, db_info: Dict, sql: str, query_config: Dict) -> List[Dict]:
        """Execute MySQL query"""
        try:
            async with db_info['pool'].acquire() as conn:
    ...
    async def _execute_sqlite_query(self, db_info: Dict, sql: str, query_config: Dict) -> List[Dict]:
    ...
    async def _create_event_from_row(self, row: Dict, query_config: Dict) -> Optional[BaseEvent]:
        """Create event from database row"""
        try:
            ...
            if event_type == EventType.AUTHENTICATION:
                return await self._create_auth_event_from_row(row, query_config, timestamp, severity)
            ...
            return BaseEvent(
                ...
                tags=['database', query_config['id'], query_config.get('category', 'generic')]
            )
    ...
    async def _determine_event_type(self, row: Dict, query_config: Dict, message: str) -> EventType:
        ...
        try:
            ...
            if 'event_type' in query_config:
                ...
                return type_mapping.get(query_config['event_type'], EventType.ACCOUNTING)
            ...
            if any(keyword in message_lower for keyword in 
                ...
                return EventType.NETWORK
                    ...
                    ['process', 'started', 'stopped', 'killed']):
    ...
    async def _create_auth_event_from_row(self, row: Dict, query_config: Dict,
                                        timestamp: datetime, 
                                        severity: Severity) -> Optional[AuthenticationEvent]:
        ...
        try:
            ...
            source_ip = row.get('source_ip') or row.get('ip_address') or row.get('client_ip')
