...
from ..models import (
    BaseEvent, AuthenticationEvent, AuthorizationEvent, ContainerEvent,
    EventType, LogSource, Severity
)
...
class KubernetesCollector:
    ...
    def __init__(self, config: Dict[str, Any], log_processor, analysis_engine):
        ...
        self.auth_patterns = {
            'failed_login': re.compile(r'authentication failed|login failed|invalid credentials', re.IGNORECASE),
            'successful_login': re.compile(r'authentication successful|login successful|authenticated', re.IGNORECASE),
            'permission_denied': re.compile(r'permission denied|access denied|forbidden|unauthorized', re.IGNORECASE),
            'token_expired': re.compile(r'token expired|token invalid|expired token', re.IGNORECASE),
            'service_account': re.compile(r'service account|serviceaccount', re.IGNORECASE),
            'rbac_violation': re.compile(r'rbac.*denied|role.*denied|rolebinding.*denied', re.IGNORECASE),
        }
        ...
        self.container_patterns = {
            ...
            'image_pull_failed': re.compile(r'image.*pull.*failed|failed.*pull.*image', re.IGNORECASE),
        }
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info("Kubernetes collector initialized successfully", 
        ...
        if api_config.token:
            ...
            try:
                ...
                logger.error("Failed to read token file", 
                           file=api_config.token_file, error=str(e))
                raise
        ...
        try:
            ...
            async with session.get(version_url) as response:
                if response.status == 200:
                    ...
                    logger.info("Kubernetes API connection successful", 
                              server=base_url, version=version_data.get('gitVersion'))
                else:
            ...
            async with session.get(ns_url) as response:
                if response.status == 200:
                    ...
                    logger.info("Kubernetes namespace access verified", 
                              server=base_url, namespace_count=ns_count)
                else:
                    logger.warning("Limited namespace access", 
                                 server=base_url, status=response.status)
                       
        except Exception as e:
            logger.error("Kubernetes API connection test failed", 
            ...
            while self.running:
                ...
                for session_id in self.api_sessions.keys():
                    ...
                    try:
                        ...
                        logger.error("Error collecting from API server", 
                                   session_id=session_id, error=str(e))
    ...
    async def _collect_logs_from_server(self, session_id: str):
        ...
        try:
            ...
            logger.error("Error collecting logs from server", 
                        session_id=session_id, error=str(e))
    
    async def _get_all_namespaces(self, session: aiohttp.ClientSession, base_url: str) -> List[str]:
        ...
        try:
            ...
            async with session.get(ns_url) as response:
                if response.status == 200:
                    ns_data = await response.json()
                    return [item['metadata']['name'] for item in ns_data.get('items', [])]
                else:
                    logger.warning("Failed to get namespaces, using default", status=response.status)
                    return ['default']
        except Exception as e:
            logger.error("Error getting namespaces", error=str(e))
            return ['default']

    async def _process_namespace_logs_api(self, session: aiohttp.ClientSession, 
                                         base_url: str, namespace: str):
        ...
        try:
            ...
            async with session.get(pods_url) as response:
                if response.status != 200:
                    logger.warning("Failed to get pods", namespace=namespace, status=response.status)
                    return
                ...
                for pod_item in pods_data.get('items', []):
                    ...
                    await self._process_pod_logs_api(session, base_url, pod_item, namespace)
                
        except Exception as e:
            logger.error("Error processing namespace logs", 
                        namespace=namespace, error=str(e))

    async def _process_pod_logs_api(self, session: aiohttp.ClientSession, base_url: str,
                                   pod_item: Dict, namespace: str):
            ...
            for container in containers:
                ...
                try:
                    await self._collect_container_logs_api(
                        session, base_url, namespace, pod_name, 
                        container_name, pod_uid
                    )
                        
                except Exception as e:
                    logger.error("Error processing pod container logs",
                               pod=pod_name, container=container_name,
                               namespace=namespace, error=str(e))
                    
        except Exception as e:
            logger.error("Error processing pod logs", pod=pod_item.get('metadata', {}).get('name'), error=str(e))
    
    async def _collect_container_logs_api(self, session: aiohttp.ClientSession, base_url: str,
                                         namespace: str, pod_name: str, container_name: str, pod_uid: str):
        ...
        try:
            ...
            logs_url = urljoin(base_url, f'/api/v1/namespaces/{namespace}/pods/{pod_name}/log')
            ...
            async with session.get(logs_url, params=params) as response:
                if response.status == 200:
                    ...
                    if logs_text.strip():
                        ...
                        await self._parse_pod_logs(logs_text, pod_name, container_name, namespace)
                        
                elif response.status == 404:
                    # Pod/container not found - might be terminated
                    logger.debug("Pod/container logs not found (terminated?)", 
                               pod=pod_name, container=container_name)
                else:
                    logger.warning("Failed to get container logs",
                                 pod=pod_name, container=container_name,
                                 status=response.status)
                        
        except Exception as e:
            logger.error("Error collecting container logs via API",
                       pod=pod_name, container=container_name, error=str(e))

    async def _parse_pod_logs(self, logs: str, pod_name: str, container_name: str, namespace: str):
        ...
        try:
            ...
            for line in log_lines:
                ...
                if event_type == EventType.AUTHENTICATION:
                    event = await self._create_auth_event(line, pod_name, container_name, namespace, timestamp)
                elif event_type == EventType.AUTHORIZATION:
                    event = await self._create_authz_event(line, pod_name, container_name, namespace, timestamp)
                elif event_type == EventType.CONTAINER_EVENT:
                    event = await self._create_container_event(line, pod_name, container_name, namespace, timestamp)
                else:
                    # Generic event
                    event = BaseEvent(
    ...
    async def _create_auth_event(self, line: str, pod_name: str, container_name: str, 
                                namespace: str, timestamp: datetime) -> AuthenticationEvent:
        ...
        # Determine success
        if any(word in line.lower() for word in ['failed', 'error', 'denied', 'invalid']):
            success = False
    ...
    async def _create_authz_event(self, line: str, pod_name: str, container_name: str,
                                 namespace: str, timestamp: datetime) -> AuthorizationEvent:
    ...
    async def _create_container_event(self, line: str, pod_name: str, container_name: str,
                                     namespace: str, timestamp: datetime) -> ContainerEvent:
        ...
        return ContainerEvent(
            ...
            severity=Severity.HIGH if action in ['failed', 'killed', 'oom_killed'] else Severity.INFO,
            message=line,
            timestamp=timestamp,
            container_name=container_name,
            pod_name=pod_name,
            ...
            while self.running:
                ...
                for session_id in self.api_sessions.keys():
                    ...
                    try:
                        ...
                        logger.error("Error watching events from API server", 
                                   session_id=session_id, error=str(e))
                ...
                async for line in response.content:
                    ...
                    try:
                        ...
                        if line_str:
                            ...
                            if 'object' in event_data and 'metadata' in event_data['object']:
                                resource_version = event_data['object']['metadata'].get('resourceVersion')
                                if resource_version:
                ...
                aaa_event = BaseEvent(
                    ...
                    raw_data={
                        'kubernetes_event': {
                            ...
                            } if k8s_event.involved_object else None
                        }
                    },
                    tags=['kubernetes', 'events', k8s_event.namespace or 'default', k8s_event.reason]
                )
            ...
            while self.running:
                ...
                for session_id in self.api_sessions.keys():
                    ...
                    try:
                        ...
                        logger.error("Error collecting audit logs from API server", 
                                   session_id=session_id, error=str(e))
            ...
            for audit_url in audit_urls:
                try:
                    ...
                    if 'since' in audit_url or 'events' in audit_url:
                        ...
                        since_time = datetime.utcnow() - timedelta(seconds=self.config.audit_log_poll_interval * 2)
                    ...
                    async with session.get(full_url, params=params) as response:
                        if response.status == 200:
                            ...
                            logger.debug("Audit endpoint returned error", 
                                       url=audit_url, status=response.status)
                            
                except Exception as e:
                    logger.debug("Error accessing audit endpoint", url=audit_url, error=str(e))
                    continue
                    
        except Exception as e:
            logger.error("Error collecting audit logs", session_id=session_id, error=str(e))
        ...
        try:
            ...
            for item in items:
                ...
                audit_line = json.dumps({
                    ...
                    'user': item.get('user', {}),
                    'timestamp': item.get('eventTime') or item.get('firstTimestamp'),
            ...
            logger.error("Error processing audit events", session_id=session_id, error=str(e))
    ...
    async def _process_audit_text(self, audit_text: str, session_id: str):
        ...
        try:
            ...
            logger.error("Error processing audit text", session_id=session_id, error=str(e))
    ...
    async def _process_audit_line(self, line: str):
        ...
        try:
            ...
            event = AuthorizationEvent(
                ...
                tags=['kubernetes', 'audit', verb, audit_data.get('namespace', 'default')]
            )
