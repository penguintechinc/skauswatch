...
class BufferManager:
    ...
    async def add_event(self, event: BaseEvent) -> bool:
        ...
        try:
            ...
            if await self._should_drop_event(event):
                ...
                logger.warning("Event dropped due to buffer overflow", 
                             event_id=event.id,
            ...
            logger.error("Error adding event to buffer", 
    ...
    async def _should_drop_event(self, event: BaseEvent) -> bool:
        ...
        try:
            ...
            total_capacity = self.max_buffer_size * len(self.event_buffers) if self.event_buffers else self.max_buffer_size
    ...
    async def _persist_critical_event(self, event: BaseEvent):
        ...
        try:
            ...
            logger.error("Error persisting critical event", 
                        event_id=event.id, error=str(e))
                ...
                for i, result in enumerate(results):
                    if isinstance(result, Exception):
                        logger.error("Batch processor error", 
                                   processor_index=i, error=str(result))
    ...
    def register_batch_processor(self, processor_func):
        ...
        logger.info("Batch processor registered", processor_count=len(self.batch_processors))
        ...
        try:
            while self.running:
                try:
                    ...
                    total_capacity = self.max_buffer_size * max(len(self.event_buffers), 1)
                    utilization = total_events / total_capacity if total_capacity > 0 else 0
                    
                    # Log warnings if utilization is high
                    if utilization > self.backpressure_threshold:
                        logger.warning("High buffer utilization detected",
        ...
        try:
            while self.running:
                try:
                    ...
                    current_buffered = sum(len(buffer) for buffer in self.event_buffers.values())
    ...
    async def stop(self):
        ...
        logger.info("Buffer manager stopped", 
                   final_stats=self.get_buffer_status())