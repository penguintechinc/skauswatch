//! SkausWatch AAA monitor service entry point — Rust port of the v1 Python
//! `services/aaa-monitor` (FastAPI). `serve` (default) runs the REST API;
...
/// SkausWatch AAA monitor service.
#[derive(Parser)]
#[command(name = "skauswatch-aaa-monitor", version)]
struct Cli {
    #[command(subcommand)]
...
async fn serve() -> anyhow::Result<()> {
    skauswatch_telemetry::init_tracing("skauswatch-aaa-monitor");
    ...
    tracing::info!(%addr, "aaa-monitor REST listening");
    readiness.set_ready();
