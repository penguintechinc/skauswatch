...
class AIAnalysisEngine:
    ...
    def __init__(
        ...
        self.consensus_strategy = ConsensusStrategy(
            config.get("consensus_strategy", "weighted")
        )
    ...
    async def analyze_batch(
        ...
        should_process = (
            force_processing
            or len(self.batch_buffer[analysis_type]) >= self.batch_config.batch_size
            or (
                datetime.utcnow() - self.batch_timestamps[analysis_type]
            ).total_seconds()
    ...
    async def _real_time_worker(self, worker_id: str):
        ...
        while self.running:
            try:
                ...
                logger.error(
                    "Real-time worker error", worker_id=worker_id, error=str(e)
                )
            ...
            try:
                ...
                for analysis_type in list(self.batch_buffer.keys()):
                    ...
                    should_process = (
                        ...
                        or batch_age >= self.batch_config.max_batch_age
                        or batch_size
                        >= (
                            self.batch_config.batch_size
                            * self.batch_config.auto_flush_threshold
                        )
                    )
    ...
    async def _process_analysis_job(self, job: AnalysisJob, worker_id: str):
        ...
        async with self.analysis_semaphore:
            ...
            try:
                ...
                logger.error("Analysis job failed", job_id=job.job_id, error=str(e))
    ...
    async def _prepare_analysis_request(self, job: AnalysisJob) -> AIAnalysisRequest:
        ...
        if job.template_name:
            template_data = {"log_data": events_data, "context": context}
    ...
    async def _get_consensus_analysis(
        ...
        providers_to_use = min(
            3, len(self.provider_manager.providers)
        )  # Use up to 3 providers

        tasks = []
        for provider_name in list(self.provider_manager.providers.keys())[
            :providers_to_use
        ]:
            task = asyncio.create_task(
                self._get_provider_response(provider_name, request)
            )
            tasks.append(task)
    ...
    async def _get_provider_response(
        self, provider_name: AIProvider, request: AIAnalysisRequest
        ...
        try:
            ...
            logger.warning(
                "Provider response failed", provider=provider_name.value, error=str(e)
            )
            return None

    async def _build_analysis_result(
        self, job: AnalysisJob, responses: List[AIAnalysisResponse]
    ) -> AnalysisResult:
    ...
    async def _build_consensus(
        self, responses: List[AIAnalysisResponse]
    ) -> Optional[AIAnalysisResponse]:
        ...
        if self.consensus_strategy == ConsensusStrategy.HIGHEST_CONFIDENCE:
            ...
            for response in responses:
                ...
                reliability = 1.0 - (
                    provider.health_metrics.error_rate if provider else 0.5
                )
    ...
    def _calculate_consensus_confidence(
        ...
        if len(confidences) > 1:
            ...
            agreement_bonus = max(
                0, (0.1 - variance) * 2
            )  # Up to 0.2 bonus for high agreement
    ...
    async def shutdown(self):
        ...
        await asyncio.gather(
            *self.workers, self.batch_processor_task, return_exceptions=True
        )
