...
class BaseAIProvider(ABC):
    ...
    def __init__(self, config: Dict[str, Any], provider_name: AIProvider):
        ...
        self.circuit_breaker_timeout = timedelta(
            minutes=config.get("circuit_breaker_timeout_minutes", 5)
        )
    ...
    async def is_rate_limited(self) -> bool:
        ...
        while (
            self.rate_limiter and (now - self.rate_limiter[0]) > self.rate_limit_window
        ):
    ...
    async def update_health_metrics(
        ...
        if success:
            ...
            if (
                self.health_metrics.consecutive_failures
        ...
        if self.health_metrics.response_times:
            self.health_metrics.average_response_time = sum(
...
class AIProviderManager:
    ...
    def __init__(
        self, config: Dict[str, Any], redis_client: Optional[aioredis.Redis] = None
    ):
        ...
        self.fallback_providers = [
            AIProvider(p) for p in config.get("fallback_providers", [])
        ]
        ...
        async with self.semaphore:
            ...
            for provider_name in providers_to_try:
                ...
                try:
                    ...
                    logger.warning(
                        "AI analysis failed, trying next provider",
                        provider=provider_name,
                        request_id=request.request_id,
                        error=str(e),
                    )
            ...
            logger.error(
                "All AI providers failed for request", request_id=request.request_id
            )
            return None
    ...
    async def _cache_response(
        self, request: AIAnalysisRequest, response: AIAnalysisResponse
    ):
        ...
        try:
            ...
            await self.redis_client.setex(
                cache_key, self.cache_ttl, json.dumps(response_data)
            )
