...
class ResponseProcessor:
    ...
    def __init__(self, redis_client=None, config: Optional[Dict[str, Any]] = None):
        ...
        # Cache configuration
        self.cache_strategy = CacheStrategy(self.config.get('cache_strategy', 'selective'))
        ...
        self.dedup_window = timedelta(minutes=self.config.get('dedup_window_minutes', 60))
        self.dedup_threshold = self.config.get('dedup_threshold', 0.9)  # Similarity threshold
    ...
    async def _parse_response(self, response: AIAnalysisResponse) -> ProcessedResponse:
        ...
        processed.completeness_score = self._calculate_completeness_score(text, response.analysis_type)
        processed.relevance_score = self._calculate_relevance_score(text, response.analysis_type)
    ...
    def _assess_quality(self, text: str, analysis_type: AIAnalysisType) -> ResponseQuality:
        ...
        structure_indicators = [
            'summary:', 'analysis:', 'findings:', 'recommendation', 
            'assessment:', 'conclusion:', 'details:'
        ]
        ...
        type_indicators = {
            AIAnalysisType.SECURITY_EVENT: ['threat', 'attack', 'malicious', 'vulnerability', 'security'],
            AIAnalysisType.ANOMALY_DETECTION: ['anomaly', 'unusual', 'outlier', 'deviation', 'abnormal'],
            AIAnalysisType.THREAT_CLASSIFICATION: ['classification', 'category', 'type', 'mitre', 'technique'],
            AIAnalysisType.LOG_CORRELATION: ['correlation', 'relationship', 'timeline', 'sequence', 'pattern'],
            AIAnalysisType.PATTERN_ANALYSIS: ['pattern', 'trend', 'frequency', 'behavior', 'distribution'],
            AIAnalysisType.INCIDENT_SUMMARY: ['incident', 'impact', 'timeline', 'response', 'remediation']
        }
        
        relevant_indicators = type_indicators.get(analysis_type, [])
        relevance_count = sum(1 for indicator in relevant_indicators if indicator in text.lower())
        score += min(0.25, relevance_count * 0.05)
        
        # Check for evidence and specificity
        if re.search(r'evidence|indicates|shows|demonstrates|reveals', text, re.IGNORECASE):
            score += 0.15
    ...
    def _calculate_completeness_score(self, text: str, analysis_type: AIAnalysisType) -> float:
        ...
        expected_elements = {
            AIAnalysisType.SECURITY_EVENT: [
                'threat', 'risk', 'impact', 'recommendation', 'evidence'
            ],
            AIAnalysisType.ANOMALY_DETECTION: [
                'anomaly', 'normal', 'deviation', 'significance', 'explanation'
            ],
            AIAnalysisType.THREAT_CLASSIFICATION: [
                'classification', 'threat', 'technique', 'indicator', 'confidence'
            ],
            AIAnalysisType.LOG_CORRELATION: [
                'correlation', 'relationship', 'timeline', 'causation', 'sequence'
            ],
            AIAnalysisType.PATTERN_ANALYSIS: [
                'pattern', 'frequency', 'trend', 'behavior', 'statistical'
            ],
            AIAnalysisType.INCIDENT_SUMMARY: [
                'incident', 'timeline', 'impact', 'response', 'remediation'
            ]
        }
    ...
    def _calculate_relevance_score(self, text: str, analysis_type: AIAnalysisType) -> float:
        ...
        relevance_keywords = {
            AIAnalysisType.SECURITY_EVENT: [
                'security', 'threat', 'attack', 'malicious', 'vulnerability',
                'breach', 'intrusion', 'unauthorized', 'suspicious'
            ],
            AIAnalysisType.ANOMALY_DETECTION: [
                'anomaly', 'abnormal', 'unusual', 'outlier', 'deviation',
                'irregular', 'unexpected', 'statistical', 'baseline'
            ],
            AIAnalysisType.THREAT_CLASSIFICATION: [
                'threat', 'malware', 'apt', 'campaign', 'actor',
                'technique', 'tactic', 'procedure', 'mitre', 'attack'
            ]
        }
        ...
        return min(1.0, keyword_count / len(keywords) * 2)  # Scale up to allow for high relevance
    ...
    def _extract_threat_level(self, text: str) -> Optional[ThreatLevel]:
        ...
        patterns = {
            ThreatLevel.CRITICAL: [
                'critical', 'severe', 'immediate', 'emergency', 'urgent',
                'critical threat', 'severe risk', 'immediate action'
            ],
            ThreatLevel.HIGH: [
                'high', 'serious', 'significant', 'major', 'important',
                'high risk', 'serious threat', 'significant risk'
            ],
            ThreatLevel.MEDIUM: [
                'medium', 'moderate', 'elevated', 'concerning', 'notable',
                'moderate risk', 'elevated threat'
            ],
            ThreatLevel.LOW: [
                'low', 'minor', 'minimal', 'slight', 'negligible',
                'low risk', 'minor threat', 'minimal impact'
            ]
        }
    ...
    def _generate_cache_key(self, response: AIAnalysisResponse) -> str:
        ...
        key_data = {
            ...
            'response_hash': hashlib.sha256(response.response_text.encode()).hexdigest()[:16]
        }
    ...
    async def _cache_response(self, cache_key: str, response: ProcessedResponse):
        ...
        if self.cache_strategy == CacheStrategy.AGGRESSIVE:
            ...
            should_cache = (
                response.quality in [ResponseQuality.EXCELLENT, ResponseQuality.GOOD] or
                response.confidence_score >= self.cache_min_confidence
            )
        elif self.cache_strategy == CacheStrategy.MINIMAL:
            should_cache = (
                response.quality == ResponseQuality.EXCELLENT and
    ...
    async def _check_deduplication(self, response: AIAnalysisResponse) -> Optional[ProcessedResponse]:
    ...
    async def _update_deduplication(self, response: AIAnalysisResponse, processed: ProcessedResponse):
        """Update deduplication tracking
    ...
    def _update_stats(self, processed: ProcessedResponse):
        ...
        self.stats['average_processing_time'] = (
            (current_avg * (total - 1) + processed.processing_time) / total
        )
