    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Anthropic rate limit hit during initialization, but provider available")
                return True
...
"""
    ...
    def _calculate_confidence(self, response_text: str, analysis_type) -> float:
        ...
        if any(keyword in response_text.lower() for keyword in 
               ['summary', 'analysis', 'assessment', 'recommendation', 'confidence']):
            confidence += 0.15
        
        # Boost for specific evidence citations
        if any(phrase in response_text.lower() for phrase in 
               ['based on the log data', 'evidence shows', 'indicates that', 'demonstrates']):
            confidence += 0.1
        ...
        if analysis_type.value == 'security_event' and any(
            ...
            ['mitre', 'att&ck', 'ioc', 'ttp', 'threat actor', 'malicious activity']
        ):
            confidence += 0.1
        
        # Boost for confidence indicators in response
        if any(phrase in response_text.lower() for phrase in 
               ['high confidence', 'confident', 'definitive', 'clearly indicates']):
            confidence += 0.05
        elif any(phrase in response_text.lower() for phrase in 
                 ['medium confidence', 'likely', 'probably', 'suggests']):
            # Neutral adjustment
            pass
        
        # Penalize for uncertainty indicators
        if any(phrase in response_text.lower() for phrase in 
               ['low confidence', 'unclear', 'uncertain', 'cannot determine', 'insufficient']):
            confidence -= 0.15
        
        # Boost for detailed technical analysis
        if len(response_text) > 500 and any(
            phrase in response_text.lower() for phrase in 
            ['technical analysis', 'forensic', 'investigation', 'methodology']
        ):
            confidence += 0.05
