...
class OllamaProvider(BaseAIProvider):
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to get model info", model=self.model, error=str(e)
                    )
                ...
                async with self.session.post(
                    ...
                    if response.status == 200:
                        ...
                        logger.error(
                            "Ollama test request failed", status=response.status
                        )
                        return False

            except aiohttp.ClientError as e:
                logger.error(
                    "Failed to connect to Ollama", url=self.base_url, error=str(e)
                )
                return False
    ...
    async def _get_models(self) -> List[Dict[str, Any]]:
        ...
        if (
            self.models_cache_time
            and (current_time - self.models_cache_time) < self.models_cache_ttl
...
"""
    ...
    def _calculate_confidence(self, response_text: str, analysis_type) -> float:
        ...
        if any(
            ...
            for keyword in ["summary:", "analysis:", "assessment:", "recommendation"]
        ):
            confidence += 0.2
        ...
        if any(
            phrase in response_text.lower()
            for phrase in ["high confidence", "confident", "certain"]
        ):
            confidence += 0.05
        elif any(
            phrase in response_text.lower()
            for phrase in ["moderate confidence", "likely", "probable"]
        ):
            # Neutral
            pass
        ...
        if any(
            phrase in response_text.lower()
            for phrase in ["uncertain", "unclear", "cannot determine", "insufficient"]
        ):
            confidence -= 0.2
