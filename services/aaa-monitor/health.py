...
class HealthChecker:
    ...
    async def _check_component_health(self, name: str, component: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Error checking component health", component=name, error=str(e))
            return {
                "status": "unhealthy",
                "error": str(e),
                "timestamp": datetime.utcnow().isoformat()
    ...
    def _analyze_component_stats(self, name: str, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if 'processing_errors' in stats and stats['processing_errors'] > 0:
                if stats['processing_errors'] > 50:
                    status = "unhealthy"
                    issues.append(f"High processing errors: {stats['processing_errors']}")
                else:
                    if status == "healthy":
                        status = "degraded"
