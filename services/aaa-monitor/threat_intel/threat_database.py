...
class QueryOptimizer:
    ...
    def optimize_query(self, query: str, params: Tuple) -> Tuple[str, Tuple, QueryOptimization]:
...
class ThreatDatabase:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info("Enhanced threat database initialized successfully",
        ...
        try:
            async with aiosqlite.connect(self.database_path) as db:
                ...
                if existing:
                    ...
                    logger.debug("Indicator updated", ioc_id=existing[0], type=ioc.type, value=ioc.value)
                    ...
                    self.stats['indicators_by_threat_level'][ioc.threat_level.value] = \
                        self.stats['indicators_by_threat_level'].get(ioc.threat_level.value, 0) + 1
                    
                    logger.debug("New indicator stored", ioc_id=ioc.id, type=ioc.type, value=ioc.value)
    ...
    async def search_indicators(self, indicator_type: str, value: str) -> List[IOC]:
        ...
        try:
            ...
            logger.error("Error searching indicators", 
                        type=indicator_type, value=value, error=str(e))
            return []

    async def _get_cached_indicator(self, indicator_type: str, value: str) -> Optional[IOC]:
    ...
    async def search_similar_indicators(self, indicator_type: str, value: str, 
                                      threshold: float = 0.8) -> List[IOC]:
    ...
    async def get_iocs(self, limit: int = 100, offset: int = 0, 
                      threat_level: Optional[ThreatLevel] = None) -> List[IOC]:
    ...
    async def delete_indicator(self, indicator_id: str) -> bool:
        ...
        try:
            async with aiosqlite.connect(self.database_path) as db:
                ...
                await db.execute('DELETE FROM matches WHERE indicator_id = ?', (indicator_id,))
                await db.commit()
                
                # Remove from cache
                if self.redis_client:
                    cache_key = f"{self.redis_prefixes['indicator']}{indicator_type}:{value}"
            ...
            logger.error("Error deleting indicator", indicator_id=indicator_id, error=str(e))
            return False
    ...
    async def set_feed_last_update(self, feed_id: str, timestamp: datetime):
        ...
        try:
            async with aiosqlite.connect(self.database_path) as db:
                ...
                ''', (feed_id, timestamp.isoformat(), datetime.utcnow().isoformat(), feed_id))
                
                await db.commit()
                
        except Exception as e:
            logger.error("Error setting feed last update", feed_id=feed_id, error=str(e))
    ...
    async def get_feed_last_update(self, feed_id: str) -> Optional[datetime]:
        ...
        try:
            ...
            logger.error("Error getting feed last update", feed_id=feed_id, error=str(e))
            return None
    ...
    async def record_match(self, match_id: str, indicator_id: str, event_id: str,
                          matched_value: str, field_name: str, confidence: float):
        ...
        try:
            while True:
                ...
                try:
                    ...
                    if expired_count > 0:
                        logger.info("Background cleanup completed", expired_count=expired_count)
    ...
    async def get_enhanced_statistics(self) -> Dict[str, Any]:
        ...
        if self.query_times:
            stats['query_performance']['avg_query_time'] = sum(self.query_times) / len(self.query_times)
