...
class FeedQualityManager:
    ...
    def assess_feed_quality(self, feed_id: str, indicators: List[IOC],
                          response_time: float, success: bool) -> float:
    ...
    def get_feed_quality(self, feed_id: str) -> Dict[str, Any]:
        ...
        return self.quality_scores.get(feed_id, {
            'overall': 0.0,
            'reliability': 0.0,
            'performance': 0.0,
            'content_quality': 0.0,
    ...
    def get_top_quality_feeds(self, limit: int = 10) -> List[Tuple[str, float]]:
        ...
        return [(feed_id, scores['overall']) for feed_id, scores in sorted_feeds[:limit]]
...
class RequestTracker:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            'avg_response_time': sum(r['duration'] for r in self.request_history) / len(self.request_history),
            'success_rate': len(successful_requests) / len(self.request_history),
            'total_requests': len(self.request_history),
            'avg_response_size': sum(r.get('response_size', 0) for r in successful_requests) / max(len(successful_requests), 1)
        }
...
class TAXIIClient:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info("Enhanced TAXII client initialized successfully",
        ...
        try:
            ...
            for feed_config in feed_configs:
                ...
                if not feed_config.get('name') or not feed_config.get('url'):
                    logger.warning("Skipping invalid feed configuration", config=feed_config)
                    continue
                ...
                feed_extra = {
                    ...
                    'auth_type': feed_config.get('auth_type', 'none'),  # none, basic, token, cert, oauth2
                    'collections_filter': feed_config.get('collections_filter', []),  # specific collections
                    'content_types': feed_config.get('content_types', ['application/stix+json']),
                    'added_after_mode': feed_config.get('added_after_mode', True),  # use added_after for incremental
                    'pagination_enabled': feed_config.get('pagination_enabled', True),
                    'max_page_size': feed_config.get('max_page_size', 1000),
                    'retry_policy': feed_config.get('retry_policy', {
                        'max_retries': 3,
                        'backoff_factor': 2.0,
                        'retry_statuses': [429, 500, 502, 503, 504]
                    }),
                    'rate_limit': feed_config.get('rate_limit', {
            ...
            logger.info("Enhanced threat feeds configured", 
                       count=len(self.feeds),
                       taxii_feeds=len([f for f in self.feeds.values() if f.feed_type == 'taxii']))
    ...
    async def _setup_authentication(self):
        ...
        try:
            ...
            logger.info("Authentication setup completed",
                       basic_auth=len([t for t in self.auth_tokens.values() if t['type'] == 'basic']),
                       token_auth=len([t for t in self.auth_tokens.values() if t['type'] == 'token']),
            ...
            for feed_id, feed in self.feeds.items():
                ...
                try:
                    ...
                    if server_url not in self.taxii_servers:
                        discovery_info = await self._perform_server_discovery(server_url, feed_id)
                        if discovery_info:
                            self.taxii_servers[server_url] = discovery_info
                            servers_discovered += 1
                            
                except Exception as e:
                    logger.error("Error discovering TAXII server", 
                               feed_name=feed.name, error=str(e))
                    continue
            
            logger.info("TAXII server discovery completed", 
                       servers_discovered=servers_discovered)
            
        except Exception as e:
            logger.error("Error in TAXII server discovery", error=str(e))
    
    async def _perform_server_discovery(self, server_url: str, feed_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with session.get(discovery_url, headers=headers) as response:
                if response.status == 200:
                    ...
                    for api_root_url in discovery_data.get('api_roots', []):
                        api_root_info = await self._discover_api_root(api_root_url, feed_id)
                        if api_root_info:
            ...
            logger.error("Error in server discovery", 
                        server_url=server_url, error=str(e))
        
        return None
    
    async def _discover_api_root(self, api_root_url: str, feed_id: str) -> Optional[Dict[str, Any]]:
            ...
            async with session.get(api_root_url, headers=headers) as response:
                if response.status == 200:
                    ...
                    return {
                        ...
                        'max_content_length': api_root_data.get('max_content_length', 10485760)
                    }
                    
        except Exception as e:
            logger.error("Error discovering API root", 
                        api_root_url=api_root_url, error=str(e))
        ...
        try:
            ...
            for feed_id, feed in self.feeds.items():
                ...
                try:
                    ...
                    logger.info("Collections discovered for feed",
                               feed_name=feed.name,
                               collection_count=len(collections))
                    
                except Exception as e:
                    logger.error("Error discovering collections for feed",
                               feed_name=feed.name, error=str(e))
            
            self.stats['collections_discovered'] = total_collections
            logger.info("Collection discovery completed",
                       total_collections=total_collections)
            
        except Exception as e:
            logger.error("Error in collection discovery", error=str(e))
    
    async def _discover_feed_collections(self, feed_id: str, feed: ThreatFeed) -> List[Dict[str, Any]]:
                ...
                if response.status == 200:
                    ...
                    if collections_filter:
                        collections = [
                            ...
                            if col.get('id') in collections_filter or 
                               col.get('title') in collections_filter
                        ]
                    
                    return collections
                else:
                    logger.error("Failed to discover collections",
                               feed_name=feed.name,
                               status=response.status)
                    
        except Exception as e:
            logger.error("Error discovering feed collections",
                        feed_name=feed.name, error=str(e))
        ...
        for feed_id, feed in self.feeds.items():
            ...
            try:
                ...
                if health_status['status'] == 'healthy':
                    ...
                    active_feeds += 1
                    logger.warning("Feed is degraded", 
                                 feed_name=feed.name,
                                 issues=health_status.get('issues', []))
                else:
                    logger.error("Feed is unhealthy", 
                               feed_name=feed.name,
                               error=health_status.get('error'))
                    
            except Exception as e:
                logger.error("Error testing feed connection", 
                           feed_name=feed.name, error=str(e))
                self.feed_health[feed_id] = {
                    'status': 'error',
                    'error': str(e),
    ...
    async def _comprehensive_health_check(self, feed_id: str, feed: ThreatFeed) -> Dict[str, Any]:
    ...
    async def _test_taxii_health(self, feed_id: str, feed: ThreatFeed) -> Dict[str, Any]:
        ...
        try:
            ...
            if not session:
                health_data['issues'].append("CRITICAL: No connection session available")
                return health_data
            ...
            try:
                async with session.get(discovery_url, headers=headers) as response:
                    if response.status == 200:
                        health_data['last_successful_request'] = datetime.utcnow().isoformat()
                        ...
                        health_data['issues'].append(f"Discovery endpoint returned {response.status}")
            ...
            for collection in collections[:3]:  # Test up to 3 collections
                ...
                if collection_id:
                    ...
                    try:
                        async with session.get(collection_url, headers=headers) as response:
                            if response.status == 200:
    ...
    async def _test_http_health(self, feed_id: str, feed: ThreatFeed) -> Dict[str, Any]:
        ...
        try:
            ...
            if not session:
                ...
                timeout = aiohttp.ClientTimeout(total=self.default_timeout)
                ssl_context = ssl.create_default_context() if feed.certificate_verification else False
        ...
        try:
            ...
            async with rate_limiter['lock']:
                ...
                rate_limiter['requests_made'] = [
                    req_time for req_time in rate_limiter['requests_made']
    ...
    async def _setup_rate_limiters(self):
        ...
        try:
            for feed_id, feed in self.feeds.items():
                enhanced_config = getattr(feed, '_enhanced_config', {})
                rate_limit_config = enhanced_config.get('rate_limit', {
                    'requests_per_minute': 60,
                    'burst_limit': 10
                })
    ...
    async def start_feed_updates(self):
        ...
        logger.info("Enhanced feed update system started", 
                ...
                try:
                    ...
                    if await self._should_update_feed(feed):
                        ...
                        if success:
                            ...
                            logger.warning("Feed update failed, backing off",
                                         feed_name=feed.name,
                                         consecutive_errors=consecutive_errors,
                                         backoff_seconds=sleep_time)
                    else:
                        # Normal interval when no update needed
                        sleep_time = min(feed.update_frequency // 4, 300)  # Check every 1/4 interval or 5 min
                    
                    await asyncio.sleep(sleep_time)
                    
                except Exception as e:
                    consecutive_errors += 1
                    logger.error("Error in enhanced feed update loop", 
                               feed_name=feed.name, error=str(e))
                    ...
                    # Progressive backoff
                    backoff_time = min(300 * consecutive_errors, 3600)  # 5min * errors, max 1 hour
                    await asyncio.sleep(backoff_time)
                    
        except asyncio.CancelledError:
            logger.info("Enhanced feed update loop cancelled", feed_name=feed.name)
        except Exception as e:
            logger.error("Fatal error in enhanced feed update loop", 
                        feed_name=feed.name, error=str(e))
    ...
    async def _calculate_adaptive_sleep_time(self, feed_id: str, update_duration: float) -> int:
    ...
    async def _resilient_feed_update(self, feed_id: str) -> bool:
        ...
        enhanced_config = getattr(feed, '_enhanced_config', {})
        retry_policy = enhanced_config.get('retry_policy', {
            'max_retries': 3,
            'backoff_factor': 2.0,
            'retry_statuses': [429, 500, 502, 503, 504]
        })
        ...
        for attempt in range(retry_policy['max_retries'] + 1):
            try:
                if attempt > 0:
                    ...
                    logger.info("Retrying feed update",
                               feed_name=feed.name,
                               attempt=attempt + 1,
                ...
                last_error = e
                logger.warning("Feed update attempt failed",
                             feed_name=feed.name,
                             attempt=attempt + 1,
                             error=str(e))
                
                # Check if this is a retryable error
                if hasattr(e, 'status') and e.status not in retry_policy['retry_statuses']:
                    logger.info("Non-retryable error, stopping attempts",
                               feed_name=feed.name,
                               status=e.status)
                    break
        
        # All attempts failed
        logger.error("All feed update attempts failed",
                    feed_name=feed.name,
    ...
    async def update_feed(self, feed_id: str) -> bool:
        ...
        async with self.update_locks[feed_id]:
            try:
                ...
                if feed.feed_type == 'taxii':
                    success, metrics = await self._update_taxii_feed_enhanced(feed_id, feed)
                elif feed.feed_type in ['json', 'xml', 'csv']:
                    success, metrics = await self._update_http_feed_enhanced(feed_id, feed)
                else:
                    logger.error("Unsupported feed type", 
                               feed_name=feed.name, feed_type=feed.feed_type)
                    return False
                ...
                self.stats['indicators_by_feed'][feed_id] += update_metrics['indicators_stored']
                self.stats['total_indicators_received'] += update_metrics['indicators_received']
                ...
                self.stats['pagination_requests'] += update_metrics['pagination_requests']
                ...
                if feed_id in self.feed_health:
                    ...
                    self.feed_health[feed_id]['last_successful_request'] = start_time.isoformat()
                ...
                logger.info("Enhanced feed update completed successfully",
                          feed_name=feed.name,
                          **update_metrics,
                ...
                logger.error("Error in enhanced feed update", 
                           feed_name=feed.name, error=str(e))
    ...
    async def _save_pagination_state(self, feed_id: str):
        ...
        try:
            ...
            if cursors:
                logger.debug("Saving pagination state", 
                           feed_id=feed_id, 
                           cursor_count=len(cursors))
        except Exception as e:
            logger.error("Error saving pagination state", 
                        feed_id=feed_id, error=str(e))

    async def _update_taxii_feed_enhanced(self, feed_id: str, feed: ThreatFeed) -> Tuple[bool, Dict[str, int]]:
    ...
    async def _update_taxii_2x_feed(self, feed_id: str, feed: ThreatFeed) -> Tuple[bool, Dict[str, int]]:
        ...
        try:
            ...
            if not session:
                logger.error("No session available for TAXII 2.x update", feed_name=feed.name)
            ...
            for collection in collections:
                ...
                try:
                    ...
                    logger.error("Error updating TAXII collection",
    ...
    async def _update_taxii_collection(self, session: aiohttp.ClientSession, 
                                     api_root_url: str, collection_id: str, 
                                     feed_id: str, headers: Dict[str, str]) -> Dict[str, int]:
        ...
        try:
            ...
            if enhanced_config.get('added_after_mode', True):
                ...
                if last_update:
                    params['added_after'] = last_update.strftime('%Y-%m-%dT%H:%M:%S.%fZ')
            
            # Add content type filters
            content_types = enhanced_config.get('content_types', ['application/stix+json'])
            if content_types:
                params['match[type]'] = ','.join([
                    'indicator', 'malware', 'attack-pattern', 'threat-actor',
                    'intrusion-set', 'campaign', 'course-of-action', 'vulnerability'
                ])
            ...
            while has_more_pages and page_count < 100:  # Safety limit
                try:
                    ...
                    async with session.get(objects_url, headers=headers, params=params) as response:
                        if response.status == 200:
                            data = await response.json()
                            ...
                            for obj in objects:
                                try:
                                    # Parse STIX object
                                    stix_indicators = await self.stix_parser._parse_stix_object(obj)
                                    metrics['indicators_processed'] += 1
                                    
                                    if stix_indicators:
                                        # Store in database
                                        success = await self.threat_database.store_indicator(stix_indicators)
                                        if success:
                                            metrics['indicators_stored'] += 1
                                            
                                            # Update type statistics
                                            indicator_type = stix_indicators.type
                                            self.stats['indicators_by_type'][indicator_type] = \
                                                self.stats['indicators_by_type'].get(indicator_type, 0) + 1
                                    
                                except Exception as e:
                                    logger.error("Error processing STIX object",
                                               collection_id=collection_id,
                                               error=str(e))
                                    continue
                            ...
                            if pagination_enabled:
                                ...
                                if more and next_cursor:
                                    ...
                                    params.pop('added_after', None)  # Remove for subsequent pages
                                    page_count += 1
                            ...
                            logger.error("Authentication failed for collection",
                                       collection_id=collection_id)
                            self.stats['authentication_failures'] += 1
                            has_more_pages = False
                        elif response.status == 429:
                            logger.warning("Rate limited for collection",
                                         collection_id=collection_id)
                            self.stats['rate_limit_hits'] += 1
                            # Wait and retry
                            ...
                            logger.error("HTTP error for collection",
                                       collection_id=collection_id,
                                       status=response.status)
                            has_more_pages = False
                        
                except asyncio.TimeoutError:
                    logger.error("Timeout updating collection", collection_id=collection_id)
                    has_more_pages = False
                except Exception as e:
                    logger.error("Error in pagination request",
                               collection_id=collection_id,
                               error=str(e))
                    has_more_pages = False
            ...
            logger.debug("Collection update completed",
                        collection_id=collection_id,
    ...
    async def _update_taxii_1x_feed(self, feed_id: str, feed: ThreatFeed) -> Tuple[bool, Dict[str, int]]:
        ...
        try:
            ...
            if not collection_service:
                logger.error("No collection management service found", feed_name=feed.name)
                return False, metrics
                ...
                try:
                    ...
                    for content_block in content_blocks:
                        try:
                            ...
                            logger.error("Error processing content block", 
                                       collection=collection.name, error=str(e))
                            continue
                    
                except Exception as e:
                    logger.error("Error polling collection", 
                               collection=collection.name, error=str(e))
                    continue
    ...
    async def _update_http_feed_enhanced(self, feed_id: str, feed: ThreatFeed) -> Tuple[bool, Dict[str, int]]:
        ...
        try:
            ...
            try:
                ...
                if feed.credentials:
                    auth_type = getattr(feed, '_enhanced_config', {}).get('auth_type', 'basic')
                ...
                # Check rate limits
                if not await self._check_rate_limit(feed_id):
                    logger.warning("Rate limit exceeded for HTTP feed", feed_name=feed.name)
                    return False, metrics
                
                async with session.get(feed.url, headers=headers, auth=auth) as response:
                    if response.status >= 400:
                    ...
                    content_length = response.headers.get('Content-Length', str(len(content)))
                    metrics['bytes_transferred'] = int(content_length)
                    ...
                    # Update metrics
                    metrics['indicators_received'] = processing_metrics['indicators_received']
                    metrics['indicators_processed'] = processing_metrics['indicators_processed'] 
                    metrics['indicators_stored'] = processing_metrics['indicators_stored']
    ...
    async def _process_http_content_enhanced(self, content: str, feed_id: str, 
                                           feed: ThreatFeed) -> Dict[str, int]:
        ...
        try:
            if feed.feed_type == 'json':
                json_metrics = await self._process_json_feed_enhanced(content, feed_id, feed)
                metrics.update(json_metrics)
            elif feed.feed_type == 'xml':
                xml_metrics = await self._process_xml_feed_enhanced(content, feed_id, feed)
                metrics.update(xml_metrics)
            elif feed.feed_type == 'csv':
                csv_metrics = await self._process_csv_feed_enhanced(content, feed_id, feed)
    ...
    async def _process_json_feed_enhanced(self, content: str, feed_id: str, 
                                        feed: ThreatFeed) -> Dict[str, int]:
        ...
        try:
                ...
                # Check for indicators array
                    ...
                    for item in indicators_data:
                        try:
                            ...
                            logger.error("Error processing indicator item", error=str(e))
            ...
            logger.error("Invalid JSON in feed content", feed_name=feed.name, error=str(e))
            return metrics
        except Exception as e:
            logger.error("Error processing enhanced JSON feed", error=str(e))
            return metrics
    
    async def _process_xml_feed_enhanced(self, content: str, feed_id: str, 
                                       feed: ThreatFeed) -> Dict[str, int]:
    ...
    async def _process_csv_feed_enhanced(self, content: str, feed_id: str, 
                                       feed: ThreatFeed) -> Dict[str, int]:
    ...
    async def _convert_to_ioc(self, data: Dict[str, Any], feed: ThreatFeed) -> Optional[IOC]:
    ...
    async def _convert_csv_to_ioc(self, row: Dict[str, str], feed: ThreatFeed) -> Optional[IOC]:
        ...
        for feed_id, feed in self.feeds.items():
            ...
            feed_statuses[feed_id] = {
                ...
                'health': {
                    ...
                    'collections_accessible': health_info.get('collections_accessible', 0),
                    'last_successful_request': health_info.get('last_successful_request')
                },
                'updates': {
                    'last_update': self.last_updates.get(feed_id),
                ...
                'statistics': {
                    'indicators_received': self.stats['indicators_by_feed'].get(feed_id, 0),
        ...
        try:
            ...
            if feed.feed_type == 'taxii' and self.enable_discovery:
                try:
                    ...
                    logger.error("Error discovering collections for new feed",
                               feed_name=feed.name, error=str(e))
            
            # Test connection and health
            if feed.enabled:
                try:
                    health_status = await self._comprehensive_health_check(feed.id, feed)
                    self.feed_health[feed.id] = health_status
                    
                    if health_status['status'] not in ['healthy', 'degraded']:
                        logger.warning("New feed health check failed", 
                                     feed_name=feed.name,
                                     status=health_status['status'])
                except Exception as e:
                    logger.error("Error testing new feed connection",
                               feed_name=feed.name, error=str(e))
            ...
            logger.info("Enhanced threat feed added", 
                       feed_name=feed.name, 
                       feed_id=feed.id,
    ...
    async def _setup_feed_authentication(self, feed_id: str):
        ...
        try:
            ...
            logger.error("Error setting up feed authentication", 
                        feed_id=feed_id, error=str(e))
    ...
    async def remove_feed(self, feed_id: str) -> bool:
        ...
        try:
            ...
            healthy_feeds = sum(1 for health in self.feed_health.values() 
                              if health.get('status') == 'healthy')
            self.stats['feeds_healthy'] = healthy_feeds
            
            logger.info("Threat feed removed successfully", 
                       feed_name=feed.name, feed_id=feed_id)
            return True
    ...
    @retry(
        ...
        retry=retry_if_exception_type((ClientError, ClientTimeout, asyncio.TimeoutError))
    )
    async def _resilient_http_request(self, session: aiohttp.ClientSession, 
