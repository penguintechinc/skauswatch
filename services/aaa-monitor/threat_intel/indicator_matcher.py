...
class IndicatorMatcher:
        ...
        try:
            ...
            if matches:
                ...
                for match in matches:
                    ...
                    self.stats["matches_by_type"][match_type] = (
                        self.stats["matches_by_type"].get(match_type, 0) + 1
                    )
            ...
            logger.debug(
                "Threat intelligence matching completed",
                event_id=event.id,
            ...
            logger.error(
                "Error matching event against indicators",
                event_id=event.id,
                error=str(e),
            )
            return []
    ...
    def _extract_from_event_fields(self, event: BaseEvent) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if hasattr(event, "source_ip") and event.source_ip:
                observables.append(
                    {"type": "ip", "value": event.source_ip, "field": "source_ip"}
                )
            ...
            if hasattr(event, "username") and event.username:
                observables.append(
                    {"type": "username", "value": event.username, "field": "username"}
                )

            # File paths
            if hasattr(event, "file_path") and event.file_path:
                observables.append(
                    {"type": "file", "value": event.file_path, "field": "file_path"}
                )

            # Processes/commands
            if hasattr(event, "command") and event.command:
                observables.append(
                    {"type": "command", "value": event.command, "field": "command"}
                )

            if hasattr(event, "executable") and event.executable:
                observables.append(
                    {"type": "file", "value": event.executable, "field": "executable"}
                )
        ...
        try:
            ...
            for ip in ip_matches:
                if self._is_valid_ip(ip):
                    observables.append(
                        {"type": "ip", "value": ip, "field": "text_content"}
                    )
            ...
            for domain in domain_matches:
                if self._is_valid_domain(domain):
                    observables.append(
                        {"type": "domain", "value": domain, "field": "text_content"}
                    )
            ...
            for email in email_matches:
                observables.append(
                    {"type": "email", "value": email, "field": "text_content"}
                )
            ...
            for url in url_matches:
                observables.append(
                    {"type": "url", "value": url, "field": "text_content"}
                )
    ...
    async def _match_observable(
        self, observable: Dict[str, Any], event: BaseEvent
    ) -> List[ThreatMatch]:
        ...
        try:
            ...
            logger.error(
                "Error matching observable", observable=observable, error=str(e)
            )
            return []

    async def _calculate_match_confidence(
        self, observable: Dict[str, Any], indicator: IOC, event: BaseEvent
    ) -> float:
    ...
    async def _fuzzy_match_observable(
        self, observable: Dict[str, Any], event: BaseEvent
    ) -> List[ThreatMatch]:
        ...
        try:
            ...
            fuzzy_indicators = await self.threat_database.search_similar_indicators(
                observable["type"], observable["value"], threshold=self.fuzzy_threshold
            )

            for indicator in fuzzy_indicators:
                # Calculate fuzzy match confidence
                similarity = await self._calculate_similarity(
                    observable["value"], indicator.value
                )
        ...
        try:
            ...
            for event_id, task in match_tasks:
                try:
                    ...
                    logger.error(
                        "Error in batch matching", event_id=event_id, error=str(e)
                    )
                    results[event_id] = []
