...
class STIXParser:
    ...
    async def _parse_indicator(self, obj: Dict[str, Any]) -> Optional[IOC]:
        ...
        try:
            ...
            # Extract confidence
            confidence = (
                obj.get("confidence", 0) / 100.0 if obj.get("confidence") else 0.5
            )
    ...
    async def _parse_indicator_pattern(self, pattern: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            for hash_type, hash_value in hash_matches:
                observables.append(
                    {"type": f"hash-{hash_type.lower()}", "value": hash_value}
                )
            ...
            reg_matches = re.findall(
                r"windows-registry-key:key\s*=\s*'([^']+)'", pattern
            )
            ...
            logger.error(
                "Error parsing indicator pattern", pattern=pattern, error=str(e)
            )
            return []
    ...
    def _extract_threat_level(self, obj: Dict[str, Any]) -> ThreatLevel:
        ...
        try:
            ...
            if kill_chains:
                ...
                if any(
                    phase in ["actions-on-objectives", "command-and-control"]
                    for phase in phase_names
                ):
                    return ThreatLevel.HIGH
                elif any(
                    phase in ["exploitation", "installation"] for phase in phase_names
                ):
                    return ThreatLevel.MEDIUM
    ...
    def _extract_registry_observables(
    ...
    async def _parse_json_content_enhanced(
            ...
            if isinstance(data, dict) and data.get("type") == "bundle":
                ...
                for obj in data:
                    try:
                        ...
                        logger.warning(
                            "Failed to parse array STIX object", error=str(e)
                        )
                return iocs
    ...
    async def parse_stix_bundle_enhanced(
        self, bundle: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> List[IOC]:
    ...
    async def _parse_stix_object_enhanced(
        self, obj: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> Optional[IOC]:
    ...
    async def _enrich_with_attack_mapping(
        self, ioc: IOC, obj: Dict[str, Any], context: Optional[Dict[str, Any]]
    ) -> IOC:
                        ...
                        attack_refs.append(
                            {
                                ...
                                "technique_name": self.mitre_attack_mapping[
                                    "common_techniques"
                                ].get(external_id),
                                "url": ref.get("url"),
            ...
            if attack_refs:
                for ref in attack_refs:
                    ...
                    if ref["technique_name"]:
                        ioc.tags.append(
                            f"technique:{ref['technique_name'].lower().replace(' ', '-')}"
                        )
