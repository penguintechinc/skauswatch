...
//! limiting — pki's REST surface, including certificate issuance and
//! revocation, previously had no request-rate defense at all; a single
//! caller could exhaust CA signing/database capacity, or brute-force a
//! bearer token, with no backpressure).
//!
//! Deliberately just plain-primitive-returning functions (`u64`/`u32`),
//! not a `governor`/`tower_governor` config builder — keeping the
//! `GovernorConfig`/`GovernorLayer` construction inline at the one call
//! site in `routes::router` means this module never has to name those
//! crates' generic types in a function signature.
//!
//! Pure `*_from` functions take `Option<&str>` (mirroring
//! `skauswatch_auth::release_mode_is_production`'s pattern) so tests never
//! touch process-global env vars — safe under `cargo test`'s default
//! parallel execution.
...
/// so no `ConnectInfo` to read). The stock `PeerIpKeyExtractor` returns
/// `GovernorError::UnableToExtractKey` (→ 500) in that case, which would
/// turn every test request into a false rate-limit failure; this extractor
/// instead degrades to coarser (whole-process) limiting, matching this
/// module's "rate limiting is defense-in-depth, never a hard dependency"
/// posture.
