//! Per-IP rate limiting (`tower_governor`) for the production `/api/v1`
//! router — env-overridable via `VAULT_RATE_LIMIT_PER_SECOND` /
//! `VAULT_RATE_LIMIT_BURST` so operators can tune per deployment without a
//! rebuild.
//!
...
//! `TestServer::new(...)` call in this crate uses) provides neither, so it
...
/// Default requests/second — `VAULT_RATE_LIMIT_PER_SECOND`.
const DEFAULT_PER_SECOND: u64 = 10;
/// Default burst size — `VAULT_RATE_LIMIT_BURST`.
const DEFAULT_BURST: u32 = 20;

/// Applies the per-IP rate limit to `router`, reading
/// `VAULT_RATE_LIMIT_PER_SECOND`/`VAULT_RATE_LIMIT_BURST` (falling back to
...
{
    let per_second = env_u64("VAULT_RATE_LIMIT_PER_SECOND", DEFAULT_PER_SECOND);
    let burst = env_u32("VAULT_RATE_LIMIT_BURST", DEFAULT_BURST);
...
mod tests {
    ...
    /// `VAULT_RATE_LIMIT_PER_SECOND`/`VAULT_RATE_LIMIT_BURST` are unset in
    /// the test process, so this also proves the unset -> default fallback
    /// path never panics and still serves traffic.
    #[tokio::test]
