...
enum Command {
    ...
    Openapi,
    /// Applies pending SQL migrations from `services/vault/migrations`
    /// against the configured database and exits — the K8s Job migration
    /// target (`skauswatch-vault migrate`). Schema authority is `sqlx
...
mod migrate_tests {
    ...
    /// `services/vault/migrations/*.sql` — see `Command::Migrate`.
