//! SkausWatch log-receiver entry point — the SIEM HTTP ingest endpoint the
//! manager's siem router proxies to (`LOG_RECEIVER_URL`). `serve` (default)
...
//! listener — are deferred; see docs/v2-port/log-receiver-contract.md.
...
/// SkausWatch log-receiver service.
#[derive(Parser)]
#[command(name = "skauswatch-log-receiver", version)]
struct Cli {
    #[command(subcommand)]
...
async fn serve() -> anyhow::Result<()> {
    skauswatch_telemetry::init_tracing("skauswatch-log-receiver");
    ...
    tracing::info!(%addr, opensearch = %cfg.opensearch_url, "log-receiver ingest listening");

    axum::serve(listener, app)
        .with_graceful_shutdown(shutdown_signal())
        .await?;
    tracing::info!("log-receiver stopped cleanly");
    Ok(())
}
