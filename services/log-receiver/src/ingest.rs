...
//! via `LOG_RECEIVER_URL`) and `GET /healthz` (the manager's liveness probe),
...
/// `GET /healthz` — v1 liveness body `{"status":"ok","service":"log-receiver"}`.
async fn handle_health() -> Response {
    ordered_json(
        ...
        &JsonVal::Obj(vec![
            ...
            (
                "service".to_owned(),
                JsonVal::Str("log-receiver".to_owned()),
            ),
        ]),
    )
}
...
mod tests {
    ...
    /// 2.7.1 (see tests/fixtures + docs/v2-port/log-receiver-contract.md).
    ...
    async fn healthz_body_matches_v1_bytes() {
        ...
        assert_eq!(res.text(), r#"{"status":"ok","service":"log-receiver"}"#);
    }
