...
//! file (`config/edr-agent.yaml`) field-for-field so existing deployments'
...
/// viper's `AddConfigPath("/etc/skauswatch")` + config name `edr-agent`.
pub const DEFAULT_CONFIG_PATH: &str = "/etc/skauswatch/edr-agent.yaml";
/// Fallback search path — v1 viper's `AddConfigPath(".")`.
pub const LOCAL_CONFIG_PATH: &str = "edr-agent.yaml";
/// Env var prefix — v1 `viper.SetEnvPrefix("EDR")` + `AutomaticEnv()`. Only
...
pub struct AgentConfig {
    ...
    /// Shared HMAC secret (manager env `EDR_API_SECRET`) used to compute the
...
pub struct ProcessCollectorConfig {
    ...
    /// process name ("edr-agent") was listed here but never actually
...
/// flag (if passed), env var (`EDR_*`), config file, built-in default.
...
mod tests {
    ...
    fn parses_shipped_yaml_file_shape() {
        let yaml = include_str!("../config/edr-agent.yaml");
    ...
    fn env_overrides_top_level_keys_only() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("EDR_MANAGER_URL", "https://override:9999");
            jail.set_env("EDR_HEARTBEAT_INTERVAL", "15");
            jail.set_env("EDR_DEBUG", "true");
    ...
    fn cli_override_wins_over_env() {
        figment::Jail::expect_with(|jail| {
            jail.set_env("EDR_MANAGER_URL", "https://from-env:1");
    ...
    fn missing_explicit_config_path_is_an_error() {
        let res = load(
            Some(Path::new("/nonexistent/edr-agent.yaml")),
