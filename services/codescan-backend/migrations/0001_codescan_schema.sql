-- CodeScan AI code-review schema (v2 Rust port).
--
-- Table names are `codescan_*`-prefixed to match the existing per-service DB
-- grant (scripts/db/init-codescan-db.sql) and the already-merged worker-codescan
-- consumer, which queries `codescan_reviews` / `codescan_repo_configs` /
-- `codescan_review_comments` directly (services/worker-codescan/src/db.rs).
...
-- (darwin/services/flask-backend/app/db_schema.py) with the `codescan_`
-- prefix applied.
--
-- No local `codescan_users` / `codescan_tenants` identity tables: authentication
...
CREATE TABLE IF NOT EXISTS codescan_repo_configs (
...
CREATE TABLE IF NOT EXISTS codescan_git_credentials (
    id                  BIGSERIAL PRIMARY KEY,
...
ALTER TABLE codescan_repo_configs
    ADD CONSTRAINT fk_codescan_repo_configs_credential
    FOREIGN KEY (credential_id) REFERENCES codescan_git_credentials (id) ON DELETE SET NULL;

CREATE TABLE IF NOT EXISTS codescan_reviews (
    id                  BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_codescan_reviews_repo_config ON codescan_reviews (repo_config_id);
CREATE INDEX IF NOT EXISTS idx_codescan_reviews_status ON codescan_reviews (status);

-- Column shape (review_id, file_path, line_number, comment, severity,
-- created_at) is a hard contract with worker-codescan's INSERT statement.
CREATE TABLE IF NOT EXISTS codescan_review_comments (
    id                      BIGSERIAL PRIMARY KEY,
    review_id               BIGINT NOT NULL REFERENCES codescan_reviews (id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_codescan_review_comments_review ON codescan_review_comments (review_id);

CREATE TABLE IF NOT EXISTS codescan_review_detections (
    id                  BIGSERIAL PRIMARY KEY,
    review_id           BIGINT NOT NULL REFERENCES codescan_reviews (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS codescan_issue_plans (
    id                      BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_codescan_issue_plans_repository ON codescan_issue_plans (repository);

CREATE TABLE IF NOT EXISTS codescan_provider_usage (
    id                  BIGSERIAL PRIMARY KEY,
    review_id           BIGINT REFERENCES codescan_reviews (id) ON DELETE SET NULL,
    provider            VARCHAR(64),
    model               VARCHAR(128),
...
CREATE TABLE IF NOT EXISTS codescan_license_policies (
    id              BIGSERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS codescan_license_detections (
    id                  BIGSERIAL PRIMARY KEY,
    review_id           BIGINT NOT NULL REFERENCES codescan_reviews (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS codescan_license_violations (
    id                  BIGSERIAL PRIMARY KEY,
    review_id           BIGINT NOT NULL REFERENCES codescan_reviews (id) ON DELETE CASCADE,
    detection_id        BIGINT REFERENCES codescan_license_detections (id) ON DELETE SET NULL,
    license_name        VARCHAR(128),
    package_name        VARCHAR(255),
    policy              VARCHAR(32),
    severity            VARCHAR(20),
