...
use skauswatch_pki_server::{grpc, health, routes, state};

/// SkausWatch PKI server.
#[derive(Parser)]
#[command(name = "skauswatch-pki-server", version)]
struct Cli {
    #[command(subcommand)]
...
async fn serve() -> anyhow::Result<()> {
    skauswatch_telemetry::init_tracing("skauswatch-pki-server");
    ...
    let app = routes::router(state.clone())
        ...
        .fallback(skauswatch_pki_server::error::fallback_not_found);
