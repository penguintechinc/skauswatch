...
pub struct SshCaConfig {
    /// OpenSSH CA private key path (`SSH_CA_KEY_PATH`, `/etc/pki/ssh_ca`).
    pub ca_key_path: String,
    /// OpenSSH CA public key path (`SSH_CA_PUBLIC_KEY_PATH`, `/etc/pki/ssh_ca.pub`).
    pub ca_public_key_path: String,
    /// Optional CA key passphrase (`SSH_CA_KEY_PASSWORD`).
    pub ca_key_password: Option<String>,
...
impl SshCaConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            ca_key_path: env_or("SSH_CA_KEY_PATH", "/etc/pki/ssh_ca"),
            ca_public_key_path: env_or("SSH_CA_PUBLIC_KEY_PATH", "/etc/pki/ssh_ca.pub"),
            ca_key_password: env_opt("SSH_CA_KEY_PASSWORD"),
...
mod tests {
    ...
    fn ssh_defaults_match_v1() {
        ...
        assert_eq!(cfg.ca_key_path, "/etc/pki/ssh_ca");
        assert_eq!(cfg.ca_public_key_path, "/etc/pki/ssh_ca.pub");
