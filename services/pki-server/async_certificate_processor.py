...
class AsyncCertificateProcessor:
    ...
    def _setup_rate_limiters(self):
        ...
        self.rate_limiters["certificate_issue"] = RateLimiter(
            RateLimitConfig(
        ...
        self.rate_limiters["ocsp_request"] = RateLimiter(
            RateLimitConfig(
    ...
    async def _start_background_processors(self):
        ...
        for i in range(2):  # 2 OCSP workers
            task = asyncio.create_task(self._ocsp_processor_worker(f"ocsp-worker-{i}"))
            self.background_tasks.append(task)
    ...
    async def get_ocsp_response(self, serial_number: str) -> Optional[bytes]:
        ...
        ocsp_response = await self._generate_ocsp_response(
            serial_number, certificate, is_revoked
        )
        ...
        while self.running:
            try:
                ...
                request = await asyncio.wait_for(
                    self.certificate_queue.get(), timeout=1.0
                )
    ...
    async def _process_certificate_request(
        self, request: CertificateRequest, worker_name: str
    ):
        ...
        try:
            ...
            self.certificates[request.request_id] = {
                **(
                    ...
                    if hasattr(certificate_data, "to_dict")
                    else certificate_data
                ),
            ...
            logger.error(
                f"Failed to process certificate request {request.request_id}: {e}"
            )
            self.metrics.error_count += 1
            raise

    @async_timeout(30.0)  # 30 second timeout
    async def _generate_certificate_async(
        ...
        return await cpu_manager.submit_computation(
            self._generate_certificate_sync, request
        )
    ...
    def _generate_certificate_sync(self, request: CertificateRequest) -> Dict[str, Any]:
            ...
            if "=" in part:
                ...
                if key.upper() == "CN":
                    subject_parts.append(
                        x509.NameAttribute(x509.NameOID.COMMON_NAME, value)
                    )
                elif key.upper() == "O":
                    subject_parts.append(
                        x509.NameAttribute(x509.NameOID.ORGANIZATION_NAME, value)
                    )
                elif key.upper() == "OU":
                    subject_parts.append(
                        x509.NameAttribute(x509.NameOID.ORGANIZATIONAL_UNIT_NAME, value)
                    )
                elif key.upper() == "C":
                    subject_parts.append(
                        x509.NameAttribute(x509.NameOID.COUNTRY_NAME, value)
                    )
        ...
        serial_number = int.from_bytes(
            hashlib.sha256(f"{request.request_id}{time.time()}".encode()).digest()[:8],
            "big",
        )
    ...
    async def _process_revocation_request(self, request: RevocationRequest):
        ...
        self.certificates[request.certificate_id][
            "status"
    ...
    async def _generate_ocsp_response(
        self, serial_number: str, certificate: Dict[str, Any], is_revoked: bool
    ) -> bytes:
        """Generate OCSP response for certificate"""
    ...
    async def _generate_crl(self):
        ...
        crl_data = await cpu_manager.submit_computation(self._generate_crl_sync)
    ...
    async def list_certificates(
        ...
        for cert_data in self.certificates.values():
            if status is None or cert_data.get("status") == status:
                ...
                safe_cert_data = {
                    k: v for k, v in cert_data.items() if k != "private_key_pem"
                }
