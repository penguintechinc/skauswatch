...
//! Streams producer used to notify `worker-icebox-sync`.
...
/// PostHog module flag gating every IceBox route (see
...
pub const ICEBOX_FLAG: &str = "skauswatch.icebox";
...
pub struct AppStateInner {
    ...
    /// Postgres pool (per-service `icebox` DB account, v1 schema —
    /// `icebox_*` tables, zero schema changes in v2.0.0).
    pub db: PgPool,
    /// Auth settings.
    ...
    /// Redis Streams producer, publishing to `icebox:sync:{provider}` for
    /// `worker-icebox-sync`. `None` only in tests.
...
impl AppStateInner {
    ...
    /// encryption exits the process if `ICEBOX_MEK` is unset and not in dev
    ...
    pub async fn from_env() -> anyhow::Result<AppState> {
        ...
        let envelope = match EnvelopeEncryption::from_env() {
            Ok(enc) => enc,
            Err(e) => {
                tracing::error!(error = %e, "encryption init failed — ICEBOX_MEK not set");
        ...
        // `skauswatch:icebox:sync:{provider}` namespace consumed by
        // `worker-icebox-sync` (see routes/sync.rs::sync_stream_name).
        let redis_url =
