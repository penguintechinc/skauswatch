//! IceBox secrets vault REST backend. `serve` (default) runs the
...
/// IceBox secrets vault service.
#[derive(Parser)]
#[command(name = "skauswatch-icebox", version)]
struct Cli {
    #[command(subcommand)]
...
async fn serve() -> anyhow::Result<()> {
    skauswatch_telemetry::init_tracing("skauswatch-icebox");
    ...
    tracing::info!(%addr, "icebox REST listening");
