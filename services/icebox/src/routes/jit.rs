...
pub async fn validate_jit_token(state: &AppState, token: &str, secret_id: &str) -> Option<String> {
    ...
    let grant = sqlx::query_as::<_, GrantRow>(
        "SELECT access_token_hash, expires_at FROM icebox_jit_grants \
...
async fn is_secret_owner(
    ...
    let count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM icebox_secret_owners \
    ...
    let rows = if has_approve {
        sqlx::query_as::<_, RequestRow>(
            "SELECT r.id, r.secret_id, r.requestor_id, r.reason, r.requested_duration_seconds, \
             ...
             r.access_expires_at, r.created_at FROM icebox_jit_requests r \
             WHERE r.requestor_id = $1 \
                OR r.secret_id IN (SELECT secret_id FROM icebox_secret_owners \
        ...
        sqlx::query_as::<_, RequestRow>(
            "SELECT id, secret_id, requestor_id, reason, requested_duration_seconds, \
             ...
             created_at FROM icebox_jit_requests WHERE requestor_id = $1 ORDER BY created_at DESC",
        )
        .bind(&user.user_id)
        .fetch_all(&state.db)
        .await?
...
async fn create_jit_request(
    ...
    let exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM icebox_secrets WHERE id = $1")
        .bind(&secret_id)
        .fetch_one(&state.db)
        .await?;
    if exists == 0 {
    ...
    sqlx::query(
        "INSERT INTO icebox_jit_requests (id, secret_id, requestor_id, reason, \
    ...
    let row = sqlx::query_as::<_, RequestRow>(
        "SELECT id, secret_id, requestor_id, reason, requested_duration_seconds, \
         approved_duration_seconds, status, approved_by, approved_at, access_expires_at, \
         created_at FROM icebox_jit_requests WHERE id = $1",
    )
    .bind(&req_id)
    .fetch_one(&state.db)
    .await?;
...
async fn approve_jit_request(
    ...
    sqlx::query(
        "INSERT INTO icebox_jit_grants (id, request_id, secret_id, grantee_id, \
    ...
    sqlx::query(
        "UPDATE icebox_jit_requests SET status = 'approved', approved_by = $1, approved_at = $2, \
...
async fn reject_jit_request(
    ...
    sqlx::query(
        "UPDATE icebox_jit_requests SET status = 'rejected', approved_by = $1, approved_at = $2 \
         WHERE id = $3",
    )
    .bind(&user.user_id)
...
mod tests {
    ...
    async fn validate_jit_token_rejects_malformed_token() {
        let state = crate::state::AppStateInner::for_tests(
            ...
            skauswatch_icebox::EnvelopeEncryption::default(),
        );
        assert!(
