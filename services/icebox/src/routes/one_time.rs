...
fn generate_token() -> (String, String) {
    let raw = skauswatch_icebox::random_32_bytes();
...
async fn create_one_time_secret(
    ...
    sqlx::query(
        "INSERT INTO icebox_one_time_secrets (id, token_hash, encrypted_value, encrypted_dek, \
...
async fn retrieve_one_time_secret(
    ...
    let row = sqlx::query_as::<_, OneTimeRow>(
        "SELECT encrypted_value, encrypted_dek, dek_version, expires_at, viewed_at \
         FROM icebox_one_time_secrets WHERE token_hash = $1",
    )
    .bind(&token_hash)
    ...
    let claimed = sqlx::query(
        "UPDATE icebox_one_time_secrets SET viewed_at = $1 \
