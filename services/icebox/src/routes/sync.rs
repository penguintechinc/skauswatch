...
//! Redis Streams for `worker-icebox-sync` (see `sync_stream_name`).
...
const VALID_DIRECTIONS: &[&str] = &["icebox_to_cloud", "cloud_to_icebox", "bidirectional"];
...
/// Unprefixed stream name consumed by `worker-icebox-sync`; the
/// `StreamProducer` prepends the shared `REDIS_KEY_PREFIX` (default
/// `skauswatch`), yielding `skauswatch:icebox:sync:{provider}`.
fn sync_stream_name(provider: &str) -> String {
    format!("icebox:sync:{provider}")
}
...
async fn list_integrations(
    ...
    let rows = sqlx::query_as::<_, IntegrationRow>(
        "SELECT id, provider, name, description, sync_direction, sync_scopes, enabled, config, \
         last_sync_at, created_at FROM icebox_cloud_integrations ORDER BY name",
    )
    .fetch_all(&state.db)
    .await?;
    Ok(Json(json!({
...
async fn create_integration(
    ...
    let sync_direction = body
        .sync_direction
        .unwrap_or_else(|| "icebox_to_cloud".to_owned());
    ...
    sqlx::query(
        "INSERT INTO icebox_cloud_integrations (id, provider, name, description, sync_direction, \
...
async fn fetch_integration(state: &AppState, id: &str) -> Result<Option<IntegrationRow>, ApiError> {
    Ok(sqlx::query_as::<_, IntegrationRow>(
        "SELECT id, provider, name, description, sync_direction, sync_scopes, enabled, config, \
         last_sync_at, created_at FROM icebox_cloud_integrations WHERE id = $1",
    )
    .bind(id)
...
async fn update_integration(
    ...
    sqlx::query(
        "UPDATE icebox_cloud_integrations SET \
...
async fn delete_integration(
    ...
    sqlx::query("DELETE FROM icebox_cloud_integrations WHERE id = $1")
        .bind(&id)
        .execute(&state.db)
        .await?;
...
mod tests {
    ...
    fn sync_stream_name_matches_v1_key_shape() {
        assert_eq!(sync_stream_name("aws"), "icebox:sync:aws");
    }
}
