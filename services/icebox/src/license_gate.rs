...
//! `licensing/validator.py::license_middleware`. Every IceBox route
//! answers 402 with the v1 body shape unless the `skauswatch.icebox`
//! PostHog flag is enabled (or the deployment domain bypasses gating), per
//! `penguin-licensing` semantics — see `services/manager/src/routes/darwin.rs`
...
/// Axum middleware: 402 `IceBox license required` unless
/// `skauswatch.icebox` evaluates enabled for this deployment.
pub async fn require_license(
    ...
    if state.license.flag_enabled(ICEBOX_FLAG).await {
