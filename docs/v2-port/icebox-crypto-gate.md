# IceBox crypto gate — verdict: PASS (byte-exact, both directions)

MERGE-BLOCKING gate for the IceBox port. Existing customer secrets were
...
- **MEK** (Master Encryption Key): 32 bytes, base64-encoded, sourced from
  `ICEBOX_MEK_V{version}` (fallback `ICEBOX_MEK`) env vars. Multiple
...
`crates/skauswatch-icebox/src/crypto.rs` (`EnvelopeEncryption`) — RustCrypto
...
`crates/skauswatch-icebox/tests/crypto_gate.rs`
...
**Result: PASS.** `cargo test -p skauswatch-icebox
...
`crates/skauswatch-icebox/examples/gen_crypto_gate_fixture.rs`:

```
$ cargo run --example gen_crypto_gate_fixture -p skauswatch-icebox
{"mek_b64": "AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8=",
 "plaintext": "s3kr1t-cross-lang-fixture-☃-🔒-value",
...
`crates/skauswatch-icebox`. Existing customer secrets encrypted by v1
remain fully readable by the v2 Rust IceBox service; MEK rotation
(`rotate_mek`) semantics — re-wrap the DEK only, never re-encrypt the
secret value — are preserved.

Proceeded to port the IceBox REST backend (`services/icebox`) and
