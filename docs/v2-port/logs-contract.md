# Logs Service — v1 Contract Spec (Rust port source of truth)

Derived from `services/logs` (aiohttp + opensearch-py 2.7.1) on
`release/v2.0.x`, 2026-07-25. The Rust port (`services/logs`, package
`skauswatch-logs`) matches this contract; deviations are documented
below with a decision.

The logs service is the SIEM log-ingest endpoint the manager's siem router
proxies to via `LOGS_URL` (see `services/manager/src/routes/siem.rs`).
...
### `GET /healthz`
- **200** `application/json`, body **`{"status":"ok","service":"logs"}`**
