...
IceBox uses **envelope encryption** to decouple key management from data encryption:
...
3. Return share URL to user:
   https://icebox.skauswatch.app/api/v1/one-time-secrets/{url_token}
...
9. Background cleanup (hourly):
   DELETE FROM icebox_one_time_secrets
...
│ POST /api/v1/secrets                         │
│  ↓ (on create/update)                        │
│ XADD icebox:sync:aws_secrets_manager         │
│      {"action": "sync", "secret_id": "..."}  │
└──────────┬───────────────────────────────────┘
           │
           ├─► XADD icebox:sync:azure_key_vault
           │
           ├─► XADD icebox:sync:gcp_secret_manager
           │
           ├─► XADD icebox:sync:oracle_oci
           │
           └─► XADD icebox:sync:k8s_secrets
               │
               ↓
           ┌────────────────────────┐
...
11 tables, all prefixed `icebox_`:
...
icebox_cloud_integrations (
  ...
  name VARCHAR(255),
  provider VARCHAR(50),
  direction VARCHAR(20),  -- icebox_to_cloud, cloud_to_icebox, bidirectional
  config JSON,
  enabled BOOLEAN,
...
├──────────────────────────────────────────────┤
│ User: icebox-flask-backend-rw                │
│ Grants: SELECT, INSERT, UPDATE, DELETE       │
│         on ALL icebox_* tables               │
│                                              │
│ User: icebox-sync-worker-rw                  │
│ Grants: SELECT, INSERT, UPDATE               │
│         on icebox_sync_events, audit_logs    │
│                                              │
│ User: icebox-webui-ro                        │
│ Grants: SELECT on secrets, audit_logs        │
│         (no write, no value decryption)      │
│                                              │
│ User: icebox-migration-admin                 │
...
db(db.icebox_secrets.tenant_id == current_tenant_id).select()

# NOT:
db(db.icebox_secrets).select()  # WRONG: leaks cross-tenant data
```
...
│  ├─ Forward to IceBox PKI (5101)
│  ├─ Add Deprecation: true header
│  ├─ Add Link: <icebox-pki-url> header
│  └─ Return response
│
└─ SSH CA (5002) — Shim Proxy
   ├─ Receives SSH cert request
   ├─ Forward to IceBox SSH CA (5102)
   ├─ Add headers + deprecation
   └─ Return response

IceBox (Port 5100)
├─ flask-backend — Full impl
├─ pki-server (5101) — From shim
├─ ssh-ca (5102) — From shim
...
- **v2.0+:** Shims removed, clients must call IceBox directly

---

**IceBox v1.0.0** | Architecture Overview | Limited AGPL-3.0
