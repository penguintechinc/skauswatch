# 🧪 IceBox Testing Guide

**Module:** IceBox Secrets Vault
**Audience:** Developers | DevOps
**Location:** `.worktrees/icebox/`
...
```bash
cd /home/penguin/code/skauswatch/.worktrees/icebox/icebox/services/flask-backend
pytest tests/ -v
```
...
```bash
cd /home/penguin/code/skauswatch/.worktrees/icebox
...
IceBox includes mock cloud providers for testing sync operations without external dependencies:
...
```bash
# Check service logs
docker logs icebox-flask-backend
docker logs icebox-sync-worker
...
- **IceBox Architecture:** `docs/icebox/CLAUDE.md`
- **SkausWatch Core Testing:** `docs/TESTING.md`
- **Test Files:** `.worktrees/icebox/icebox/services/flask-backend/tests/`
- **Smoke Runner:** `.worktrees/icebox/icebox/tests/smoke/run-all.sh`
