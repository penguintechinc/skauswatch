# IceBox — Deployment & Usage Guide

**Audience:** DevOps | Developers

## Prerequisites

- SkausWatch v1.x with IceBox sub-module (`.worktrees/icebox/icebox/`)
...
- PenguinTech license key with `icebox` feature enabled
- kubectl and helm (or kustomize) configured

## Enable IceBox in SkausWatch
...
```bash
# IceBox Integration
ICEBOX_ENABLED=true                              # Enable IceBox module
ICEBOX_PKI_URL=http://icebox-pki-server:5101    # PKI shim target
ICEBOX_SSH_CA_URL=http://icebox-ssh-ca:5102     # SSH CA shim target
ICEBOX_MEK=<base64-encoded-32-byte-key>         # Master Encryption Key for DEK wrapping
```

### 2. Deploy IceBox Services

#### Option A: Via Helm (Beta/Production)

```bash
# Add IceBox charts to deployment
helm upgrade --install icebox-flask-backend \
...
# Repeat for sync-worker, pki-server, ssh-ca, webui
...
```bash
# Deploy all IceBox services to local K8s
...
kubectl --context local-alpha exec \
  -it deployment/icebox-flask-backend -n icebox \
...
curl -H "Host: icebox.skauswatch.localhost.local" \
...
**Share URL:** `https://icebox.skauswatch.app/api/v1/one-time-secrets/ots:abcd1234efgh5678ijkl9012mnop3456`
...
The IceBox worktree includes its own comprehensive docs:

| File | Location |
|------|----------|
| **DEVELOPMENT.md** | `.worktrees/icebox/icebox/docs/DEVELOPMENT.md` |
| **TESTING.md** | `.worktrees/icebox/icebox/docs/TESTING.md` |
| **PRE_COMMIT.md** | `.worktrees/icebox/icebox/docs/PRE_COMMIT.md` |
...
**IceBox v1.0.0** | Production-ready | Limited AGPL-3.0
