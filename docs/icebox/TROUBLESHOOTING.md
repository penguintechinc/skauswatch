...
# or
kubectl get pods --context local-alpha -n icebox | grep postgres
...
2. **Check database service is ready:**
   ...
   - For Docker Compose: `docker logs icebox-postgres`

3. **Verify network connectivity (Docker Compose):**
   ```bash
   docker network ls | grep icebox
   docker network inspect <icebox-network>
   ```

4. **Reset database (development only):**
   ```bash
   # Drop and recreate (WARNING: data loss)
   psql -h $DB_HOST -U postgres -c "DROP DATABASE IF EXISTS icebox_db;"
   psql -h $DB_HOST -U postgres -c "CREATE DATABASE icebox_db OWNER icebox_app;"

   # Run migrations
   docker exec icebox-flask-backend \
...
5. **Check per-service account permissions:**
   ...
   -- PostgreSQL
   \du icebox_app
   GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO icebox_app;

   -- MySQL
   SHOW GRANTS FOR 'icebox_app'@'%';
   ```
...
**Diagnosis:**
```bash
echo "MEK length: $(echo -n $ICEBOX_MEK | base64 -d | wc -c)"
...
2. **Verify MEK format:**
   ...
   - Decode and check length: `base64 -d <<< "$ICEBOX_MEK" | wc -c` → 32

3. **Check MEK version matches:**
   ```bash
   echo "ICEBOX_MEK_VERSION=$ICEBOX_MEK_VERSION"
   # Must match version in database table icebox_mek_rotations
   ```

4. **Clear corrupted cache (if using Redis):**
   ```bash
   redis-cli -h $REDIS_HOST DEL icebox:*
   ```
...
docker exec icebox-flask-backend \
  python3 -c "
from icebox.services.flask_backend.models.db import db
from icebox.services.flask_backend.models.db import icebox_mek_rotations
rows = db(icebox_mek_rotations).select()
...
1. **Rotate MEK securely:**
   ...
   curl -X POST https://icebox.localhost.local/api/v1/admin/mek/rotate \
...
3. **Verify old MEK versions are still available:**
   - Check `icebox_mek_rotations.retired_at` — should be NULL for active versions
...
2. **Check token not already used:**
   ...
   docker exec icebox-flask-backend \
     python3 -c "
from icebox.services.flask_backend.models.db import db, icebox_jit_grants
rows = db(icebox_jit_grants).select()
...
3. **Regenerate new token if needed:**
   ...
   curl -X POST https://icebox.localhost.local/api/v1/jit/request \
...
docker exec icebox-flask-backend \
  python3 -c "
from icebox.services.flask_backend.models.db import db, icebox_jit_grants
row = db(icebox_jit_grants.id == 'grant-001').select().first()
...
2. **Approve pending grant (if awaiting approval):**
   ```bash
   curl -X POST https://icebox.localhost.local/api/v1/jit/approve \
...
3. **Check grant status:**
   ...
   curl -H "Authorization: Bearer $JWT" \
     https://icebox.localhost.local/api/v1/jit/{grant_id}
   ```
...
docker exec icebox-flask-backend \
  python3 -c "
from icebox.services.flask_backend.models.db import db, icebox_one_time_secrets
row = db(icebox_one_time_secrets.id == 'ots-001').select().first()
if row:
...
1. **Create new one-time secret:**
   ```bash
   curl -X POST https://icebox.localhost.local/api/v1/one-time \
...
curl -v https://icebox.localhost.local/api/v1/secrets \
...
3. **Refresh expired token:**
   ...
   curl -X POST https://icebox.localhost.local/api/v1/auth/refresh \
...
# Example: GET /api/v1/secrets requires 'icebox:secrets:read' scope
```
...
2. **Use service account with higher scopes:**
   - Service-to-service calls can use service account JWT with `icebox:*` scope

3. **Check role mapping:**
   - admin role → `icebox:*:*` (all operations)
   - maintainer role → `icebox:secrets:read icebox:secrets:write icebox:jit:request`
   - viewer role → `icebox:secrets:read icebox:audit:read`
...
kubectl get pods --context local-alpha -n icebox | grep sync-worker
# or
docker ps | grep sync-worker

# Check Redis stream has events
redis-cli XLEN icebox:cloud-sync
...
1. **Verify cloud provider credentials:**
   ...
   kubectl get secret icebox-cloud-creds --context local-alpha -n icebox -o yaml
   ```

2. **Restart sync-worker:**
   ```bash
   kubectl rollout restart deployment/sync-worker --context local-alpha -n icebox
   ```

3. **Check sync-worker logs:**
   ```bash
   kubectl logs -f deployment/sync-worker --context local-alpha -n icebox
   # Look for: "Failed to sync to AWS" or provider-specific errors
   ```

4. **Manually trigger sync for specific provider:**
   ```bash
   curl -X POST https://icebox.localhost.local/api/v1/admin/sync \
     -H "Authorization: Bearer $ADMIN_JWT" \
     -H "Content-Type: application/json" \
...
4. **Validate license directly:**
   ...
   curl -X POST $LICENSE_SERVER_URL/api/v2/validate \
     -H "Content-Type: application/json" \
     -d "{\"license_key\": \"$LICENSE_KEY\", \"product\": \"icebox\"}"
   ```
...
**Symptom:** Blank page or "Failed to load" on login at `https://icebox.localhost.local/`

**Diagnosis:**
```bash
# Check WebUI container is running
docker ps | grep webui
kubectl get pods --context local-alpha -n icebox | grep webui

# Check browser console for errors (F12 → Console tab)

# Check WebUI logs
docker logs icebox-webui
# or
kubectl logs deployment/webui --context local-alpha -n icebox
```
...
1. **Verify WebUI environment variables:**
   ...
   env:
     - name: VITE_API_URL
       value: "https://icebox.localhost.local"  # or backend IP:port
   ```

2. **Check API connection:**
   ```bash
   # From WebUI container, test API reachability
   curl -I https://icebox-flask-backend:5000/api/v1/status
   ```
...
curl -I https://icebox.localhost.local/api/v1/secrets

# Check CORS headers are present
curl -I -H "Origin: https://icebox.localhost.local" \
  https://icebox.localhost.local/api/v1/secrets | grep "Access-Control"
```
...
1. **Verify API is running:**
   ...
   kubectl get pods --context local-alpha -n icebox | grep flask-backend
   ```

2. **Check CORS_ORIGINS environment variable:**
   ```bash
   # Flask-Backend must allow WebUI origin
   CORS_ORIGINS="https://icebox.localhost.local"
...
4. **Verify certificate if using HTTPS:**
   ```bash
   curl -k https://icebox.localhost.local/api/v1/status  # Skip cert validation with -k
   ```
...
1. **Flask-Backend logs:**
   ```bash
   docker logs icebox-flask-backend 2>&1 | tail -100
   # or
   kubectl logs deployment/flask-backend --context local-alpha -n icebox --tail=100
   ```
...
5. **Database logs (if using Docker):**
   ```bash
   docker logs icebox-postgres 2>&1 | tail -50
   ```
...
| `cryptography.fernet.InvalidToken` | Wrong MEK or corrupted data | Verify ICEBOX_MEK matches storage |
...
- **IceBox GitHub Issues**: https://github.com/penguintechinc/skauswatch/issues

---

**Last Updated:** 2026-03-10
**IceBox Version:** 1.0.0
