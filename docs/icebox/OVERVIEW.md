# IceBox — Enterprise Secrets Vault Sub-Module
...
**Domain:** `icebox.skauswatch.app` (production) | `icebox.skauswatch.penguintech.cloud` (beta) | `icebox.skauswatch.localhost.local` (alpha)

---

## Quick Reference: IceBox Services
...
| **pki-server** | 5101 | Python 3.13 | Quart | X.509 certificate management (from core PKI shim) |
| **ssh-ca** | 5102 | Python 3.13 | Quart | SSH certificate authority (from core SSH CA shim) |
| **sync-worker** | — | Python 3.13 | Celery/asyncio | Cloud vault sync (AWS/Azure/GCP/OCI/K8s) |
| **webui** | 3100 | Node.js 18+ | React/Vite | IceBox vault management UI |
...
| **[USAGE.md](./USAGE.md)** | Deploying IceBox, enabling in SkausWatch, common workflows | DevOps, Developers |
...
IceBox is **integrated with the core SkausWatch platform** while remaining an optional licensed module:

### Shim Proxies
- **PKI Server** (SkausWatch core, port 5001) → proxies to IceBox PKI (port 5101)
- **SSH CA** (SkausWatch core, port 5002) → proxies to IceBox SSH CA (port 5102)
- Both shims include `Deprecation:` and `Link:` headers (RFC 8594)
- Shims to be removed in v2.0.0

### Environment Variables (Core)
```bash
ICEBOX_PKI_URL=http://icebox-pki-server:5101    # PKI endpoint
ICEBOX_SSH_CA_URL=http://icebox-ssh-ca:5102     # SSH CA endpoint
```

### Namespace Isolation
- **Core namespace:** `skauswatch`
- **IceBox namespace:** `icebox`
...
IceBox validates licensing on **every API request**:
...
3. If license valid and `icebox` feature enabled → allow
...
- **Related docs:** See [../core/](../core/) for SkausWatch core, [../darwin/](../darwin/) for Darwin AI

---

**IceBox v1.0.0** | Implemented Phases 1–10 | Production-ready | License: Limited AGPL-3.0
