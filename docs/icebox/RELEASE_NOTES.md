# IceBox Release Notes

## 📌 Version History

This document tracks all releases of the IceBox secrets vault sub-module for SkausWatch.
...
**Branch:** `v1.x` (IceBox module at `.worktrees/icebox/`)
...
| pki-server | Go 1.24 | 5001 (shim proxy) | ✅ Compatibility |
| ssh-ca | Go 1.24 | 5002 (shim proxy) | ✅ Compatibility |
...
|-------|---------|---------|
| `icebox_secrets` | Secret metadata | Per-secret (name, description, tags) |
| `icebox_secret_versions` | Versioned DEK/EDEK | Per-rotation |
| `icebox_secret_values` | Encrypted values | Per-version |
| `icebox_jit_grants` | JIT access grants | Per-request |
| `icebox_one_time_secrets` | One-time links | Per-creation |
| `icebox_cloud_syncs` | Cloud integration config | Per-provider |
| `icebox_audit_log` | Full audit trail | Per-action |
| `icebox_mek_rotations` | MEK version history | Per-rotation |
| `icebox_sync_state` | Cloud sync progress | Per-provider |
| `icebox_license_cache` | License validation cache | Single row |
...
# Domain: https://icebox.localhost.local
```

**Beta:**
```bash
helm upgrade --install icebox ./k8s/helm/flask-backend \
  --kube-context dal2-beta --namespace icebox \
  --values k8s/helm/flask-backend/values-beta.yaml
# Domain: https://icebox.penguintech.cloud
```

**Production:**
```bash
helm upgrade --install icebox ./k8s/helm/flask-backend \
  --kube-context skauswatch-prod --namespace icebox \
  --values k8s/helm/flask-backend/values-prod.yaml
# Domain: https://icebox.nestdata.app
```
...
1. **Shim Proxies:** PKI Server and SSH-CA run as thin Quart shims with Deprecation headers. **Will be removed in v2.0** when SkausWatch PKI is fully integrated into IceBox.
...
**Bugs & Issues:**
- GitHub: https://github.com/penguintechinc/skauswatch/issues (tag: `icebox`)
...
**Development Team:**
- IceBox Architecture & Core (Phases 1–6)
