# CodeScan - Configuration Reference
...
CodeScan configuration is managed entirely through environment variables. All variables are optional unless marked **REQUIRED**.

### AI Provider Selection

| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `CODESCAN_AI_PROVIDER` | ✅ YES | — | AI backend to use: `claude`, `openai`, or `ollama` |
| `CODESCAN_FALLBACK_PROVIDER` | ❌ NO | — | Fallback provider if primary fails |

---

## 🤖 Anthropic Claude Configuration

**Required when**: `CODESCAN_AI_PROVIDER=claude`
...
**Required when**: `CODESCAN_AI_PROVIDER=openai`
...
**Required when**: `CODESCAN_AI_PROVIDER=ollama`
...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `CODESCAN_GITHUB_WEBHOOK_SECRET` | ✅ YES | — | Secret for GitHub webhook validation (minimum 32 chars) |
| `CODESCAN_GITLAB_WEBHOOK_SECRET` | ✅ YES | — | Secret for GitLab webhook validation (minimum 32 chars) |
...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `CODESCAN_MAX_REVIEWS_PER_DAY` | ❌ NO | `1000` | Daily review limit per tenant |
| `CODESCAN_MAX_MONTHLY_COST_UUSD` | ❌ NO | `100000` | Monthly cost limit in 1/100th USD (default: $1000) |
| `CODESCAN_RATE_LIMIT_REQUESTS_PER_MINUTE` | ❌ NO | `60` | API rate limit (requests/minute) |
...
| `REDIS_KEY_PREFIX` | ❌ NO | `codescan:` | Key prefix for CodeScan data |
...
# Webhooks
CODESCAN_GITHUB_WEBHOOK_SECRET=local-dev-secret-12345678901234567890
CODESCAN_GITLAB_WEBHOOK_SECRET=local-dev-secret-12345678901234567890
...
# Webhooks
CODESCAN_GITHUB_WEBHOOK_SECRET=generated-secure-32-character-secret-xxxx
CODESCAN_GITLAB_WEBHOOK_SECRET=generated-secure-32-character-secret-xxxx
...
# Cost Controls
CODESCAN_MAX_REVIEWS_PER_DAY=500
CODESCAN_MAX_MONTHLY_COST_UUSD=50000  # $500/month
...
CODESCAN_MAX_MONTHLY_COST_UUSD=10000  # $100/month
...
python3 -c "
import os
required = ['CODESCAN_AI_PROVIDER', 'CODESCAN_GITHUB_WEBHOOK_SECRET',
            'CODESCAN_GITLAB_WEBHOOK_SECRET', 'DB_HOST', 'REDIS_URL']
...
| `CODESCAN_MAX_REVIEWS_PER_DAY` | ❌ NO | Next day uses new limit |
| `CODESCAN_MAX_MONTHLY_COST_UUSD` | ❌ NO | Next check uses new limit |
| `CODESCAN_AI_PROVIDER` | ✅ YES | Provider change requires restart |
