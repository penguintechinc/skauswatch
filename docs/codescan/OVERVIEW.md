# CodeScan - AI-Powered Code Review Module
...
CodeScan is a licensed AI-powered code review and issue planning sub-module for SkausWatch. It automatically analyzes pull requests and issues on GitHub and GitLab, providing intelligent feedback via configured AI providers (Claude, OpenAI, or self-hosted Ollama). CodeScan helps teams maintain code quality, security, and consistency at scale.
...
## 🏗️ CodeScan Services & Quick Reference

| Service | Port | Language | Framework | Purpose |
|---------|------|----------|-----------|---------|
| **Flask Backend** (`services/worker-codescan/`) | 5000 | Python 3.13 | Flask/Quart | REST API, webhook handlers, review orchestration |
| **Celery Worker** (`services/worker-codescan-celery/`) | — | Python 3.13 | Celery | Async review processing, plan generation |
...
CodeScan analyzes code across four configurable categories:
...
CodeScan supports multiple AI backends for flexibility:
...
- **License Server** — Feature gating (`codescan` feature required)

---

## 📂 Documentation Index

Complete CodeScan documentation is organized by topic. Start with your role:

### 👨‍💻 For Developers

| File | Purpose |
|------|---------|
| [**USAGE.md**](./USAGE.md) | Getting started, prerequisites, webhook configuration, starting CodeScan |
...
helm upgrade --install codescan ./k8s/helm/codescan \
  ...
  --values ./k8s/helm/codescan/values-beta.yaml
```
...
5. **Secret**: Set `CODESCAN_GITHUB_WEBHOOK_SECRET` in CodeScan config

### 4. Test Review

Open a PR in your repository. CodeScan will automatically analyze it and post comments.

---

## 🔐 License & Availability

**License Requirement**: CodeScan requires the `codescan` feature in your PenguinTech license key.
...
CodeScan tracks the following metrics:
