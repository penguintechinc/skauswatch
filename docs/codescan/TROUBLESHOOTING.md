# CodeScan - Troubleshooting Guide
...
### Symptom
GitHub/GitLab pushes events, but CodeScan doesn't create reviews.
...
echo $CODESCAN_GITHUB_WEBHOOK_SECRET
...
| Issue | Solution |
|-------|----------|
| **Webhook secret mismatch** | Update secret in GitHub to match `CODESCAN_GITHUB_WEBHOOK_SECRET`, or vice versa |
| **Wrong endpoint URL** | Verify `https://your-codescan/api/v1/webhooks/github` is correct |
| **CodeScan not running** | Check service logs: `kubectl logs -l app=codescan-backend -n skauswatch` |
| **Network unreachable** | Verify GitHub can reach your CodeScan instance (check firewall rules, DNS) |
| **CORS/TLS errors** | Ensure CodeScan has valid HTTPS certificate |
...
kubectl set env deployment/codescan-backend LOG_LEVEL=debug -n skauswatch

# Tail logs
kubectl logs -f -l app=codescan-backend -n skauswatch | grep -i webhook
...
```bash
# Tail worker logs
kubectl logs -f -l app=codescan-worker -n skauswatch
...
| Issue | Solution |
|-------|----------|
| **Missing GitHub token** | Set `GITHUB_API_TOKEN` env var in CodeScan backend |
...
| **Celery worker not running** | Check: `kubectl get pods -n skauswatch -l app=codescan-worker` |
...
```bash
# View pending jobs
redis-cli llen codescan:queue:review

# Check if workers are processing
kubectl logs -l app=codescan-worker -n skauswatch | grep "Processing"
```
...
```bash
# For Claude
kubectl logs -l app=codescan-worker | grep -i "rate limit\|quota"
...
| **Rate limit** | Upgrade API plan, reduce `CODESCAN_MAX_REVIEWS_PER_DAY` |
| **Queue overloaded** | Scale Celery workers: `kubectl scale deployment/codescan-worker --replicas=3` |
| **Large diff** | Increase `REVIEW_MAX_DIFF_SIZE_KB` or summarize diff first |
| **Network latency** | Verify CodeScan → AI provider network latency |
...
kubectl logs -l app=codescan-worker -n skauswatch | \
...
| **Analyzing too many reviews** | Reduce `CODESCAN_MAX_REVIEWS_PER_DAY` or scope to high-value repos only |
...
1. **Increase limit** (temporary):
   ...
   kubectl set env deployment/codescan-backend \
     CODESCAN_MAX_MONTHLY_COST_UUSD=100000 \
     -n skauswatch
   ```
...
```bash
# From CodeScan pod, test connectivity
kubectl exec -it pod/codescan-backend-xxx -n skauswatch -- \
...
```bash
# Logs by pod
kubectl logs -l app=codescan-backend -n skauswatch --tail=10000 | wc -l

# Archive old logs
kubectl logs -l app=codescan-backend -n skauswatch \
  --timestamps=true --all-containers=true > /tmp/codescan-logs.txt
```
...
```bash
# View metrics
curl http://localhost:9090/metrics | grep codescan

# Key metrics:
codescan_reviews_total              # Total reviews
codescan_reviews_duration_seconds   # Processing time
codescan_ai_tokens_used             # Token consumption
codescan_celery_queue_size          # Pending jobs
codescan_webhook_errors_total       # Webhook failures
```
...
```bash
# Collect all logs
mkdir -p /tmp/codescan-diagnostics

# Backend logs
kubectl logs -l app=codescan-backend -n skauswatch > \
  /tmp/codescan-diagnostics/backend.log

# Worker logs
kubectl logs -l app=codescan-worker -n skauswatch > \
  /tmp/codescan-diagnostics/worker.log

# Database logs
kubectl logs -l app=postgres -n skauswatch > \
  /tmp/codescan-diagnostics/postgres.log

# Recent reviews
kubectl exec -it pod/postgres-xxx -n skauswatch -- \
  psql -U skauswatch -d skauswatch -c \
  "SELECT id, status, error_message, created_at FROM reviews ORDER BY created_at DESC LIMIT 50;" > \
  /tmp/codescan-diagnostics/recent-reviews.txt

# Config
env | grep CODESCAN > /tmp/codescan-diagnostics/config.env

# Share with support
tar -czf codescan-diagnostics.tar.gz /tmp/codescan-diagnostics/
