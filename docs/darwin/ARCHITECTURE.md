...
Darwin integrates with SkausWatch as a licensed sub-module. It receives webhook events from GitHub and GitLab, processes code diffs through AI providers, and posts review comments back to pull requests/merge requests.
...
- Secret: `DARWIN_GITHUB_WEBHOOK_SECRET`
...
- Secret: `DARWIN_GITLAB_WEBHOOK_SECRET`
...
Darwin uses PostgreSQL tables (shared with SkausWatch):
...
| `darwin_user_id` | UUID | Darwin user ID |
...
Darwin constructs context-specific prompts for each review category:
...
kubectl set env deployment/darwin-backend DARWIN_AI_PROVIDER=openai
```
...
Darwin API endpoints use JWT tokens:

```bash
# Request
curl -H "Authorization: Bearer <token>" \
     https://darwin.example.com/api/v1/reviews
...
Darwin tracks AI spending per user/team:
...
if current_cost > DARWIN_MAX_MONTHLY_COST_UUSD:
...
Darwin uses Celery with Redis for async processing:

**Task Queue Structure**:

```
Redis Queue
├─ darwin:review:security
├─ darwin:review:best_practices
├─ darwin:review:framework
├─ darwin:review:iac
└─ darwin:plan:generation
...
Darwin exports Prometheus metrics:

| Metric | Type | Purpose |
|--------|------|---------|
| `darwin_reviews_total` | Counter | Total reviews processed |
| `darwin_reviews_duration_seconds` | Histogram | Review processing time |
| `darwin_ai_tokens_used` | Counter | Tokens sent to AI provider |
| `darwin_cost_uusd` | Gauge | Monthly cost tracking |
| `darwin_webhook_errors_total` | Counter | Failed webhook validations |
| `darwin_celery_queue_size` | Gauge | Jobs waiting in queue |
...
Darwin shares infrastructure with SkausWatch:
...
| **User Management** | ✅ Yes | Darwin users are SkausWatch users |
| **Audit Logging** | ✅ Yes | Logged to SkausWatch audit table |
| **License Server** | ✅ Yes | Darwin features gated by license |
...
```bash
kubectl scale deployment/darwin-backend --replicas=3
```

**Celery Workers**: Independent workers, can scale per task type
```bash
kubectl scale deployment/darwin-worker-security --replicas=2
kubectl scale deployment/darwin-worker-planning --replicas=1
```
