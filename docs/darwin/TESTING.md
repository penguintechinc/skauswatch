...
Darwin testing covers three levels:
...
Smoke tests verify Darwin can start, respond to health checks, and process basic operations.
...
```bash
docker build -t darwin-test:latest ./services/worker-darwin
docker build -t darwin-webui-test:latest ./services/webui
```
...
docker-compose up -d postgres redis darwin-backend darwin-worker

# Check logs
docker-compose logs darwin-backend | head -50

# Verify running
docker-compose ps | grep darwin
```
