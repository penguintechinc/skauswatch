...
Darwin exposes two webhook receivers: one for GitHub, one for GitLab. Both use HMAC-SHA256 signature verification.
...
```bash
curl -X POST https://your-darwin/api/v1/webhooks/github \
...
| 400 | Repository not configured in Darwin |
...
```bash
curl -X POST https://your-darwin/api/v1/webhooks/gitlab \
  -H "Content-Type: application/json" \
...
curl -H "Authorization: Bearer eyJhbGc..." \
     https://your-darwin/api/v1/reviews
```
...
**Example Output**:
```
darwin_reviews_total{platform="github"} 1234
darwin_reviews_duration_seconds_sum{platform="github"} 3456.7
darwin_reviews_duration_seconds_count{platform="github"} 123
darwin_ai_tokens_used{provider="claude"} 456789
darwin_cost_uusd 12345
```
