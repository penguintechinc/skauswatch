-- Golden parity harness seed for the FROZEN v1 side (skauswatch_v1 database).
-- Identical row-for-row to seed_v2.sql except the endpoint-agent tables keep
-- their pre-rename names (`edr_agents`/`edr_events`) — v1 is a frozen
-- snapshot of release/v1.0.x and its queries were never renamed, so its
-- database must keep the names those queries expect. See docs/MIGRATION.md
-- for the full module rename map.
--
